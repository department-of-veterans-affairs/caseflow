...
module WarRoom
    class CancelActiveTaskArray
        def run(assigned_to_id, task_type)
            ...
            if User.find_by_id(user).nil? == true;
            else
            end
            # Show the organization if found.
            if Organization.find_by_id(user).nil? == false;
            else
            ...
            if User.find_by_id(user).nil? == false || Organization.find_by_id(user).nil? == false;
            else
            ...
            if (User.find_by_id(user).nil? == false || Organization.find_by_id(user).nil? == true) &&
            ...
            array_task_ids=Task.where(assigned_to_id: user,
                type: task_type, status: ["on_hold", "in_progress", "assigned"]);
                puts ("#{array_task_ids}");
            ...
            array_task_ids=Task.where(assigned_to_id: user, type: task_type, status: ["on_hold", "in_progress", "assigned"]);
