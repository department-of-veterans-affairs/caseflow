...
class AssocationWrapper
  ...
  def except_fieldnames(ignore_fieldnames)
    if ignore_fieldnames.any?
      @selects << ->(assoc) { ! ignore_fieldnames&.include?(assoc.foreign_key) }
  ...
  def fieldnames
    selected_associations.map(&:foreign_key)
  end

  def selected_associations
    @associations.select { |assoc| @selects.map{|l| l.call(assoc)}.all? }
  end
  ...
  def typed_associations(excluding: [])
    belongs_to.having_type_field.except_fieldnames(excluding).associations!
  end

  def typed_associations_with(assoc_class)
    belongs_to.having_type_field.associated_with_type(assoc_class).associations!
  end

  def untyped_associations_with(assoc_class)
    belongs_to.without_type_field.associated_with_type(assoc_class).associations!
  end
  ...
  def grouped_fieldnames_of_typed_associations_with(known_classes)
    ...
    belongs_to.associations!.associations.group_by(&:class_name)
      .slice(*known_classes)
