...
module WarRoom
  class UnrecognizedAppellant
    def self.run(dispatch_task_id)
      ...
      # Temporarily unblock the user by unassigning the task from the user and putting
      # it back to the bottom of the queue of dispatch tasks to get assigned
      # by setting the created_at time to now we put the task to the end of
