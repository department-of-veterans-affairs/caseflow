class TaskReassignment
    def self.by_task_id(old_css_id_string, new_css_id_string, array_of_task_ids)
    ...
    def self.by_appeal_uuid(old_css_id_string, new_css_id_string, array_of_uuids_as_strings)
        #Set the current user so that Paper Trail can record the user that made changes
        RequestStore[:current_user] = User.system_user
    ...
    def
        self.all_active_tasks(old_css_id_string, new_css_id_string)
        #Set the current user so that Paper Trail can record the user that made changes
        RequestStore[:current_user] = User.system_user
        ...
        #t=LegacyAppeal.find(legacy_task_params[:appeal_id]
