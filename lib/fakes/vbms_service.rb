...
class Fakes::VBMSService
  ...
  def self.fetch_documents_for(appeal, _user = nil)
    ...
    docs = (document_records || {})[appeal.vbms_id] || @documents || []

    {
      manifest_vbms_fetched_at: @manifest_vbms_fetched_at || Time.now.strftime('%D %l:%M%P %Z'),
      manifest_vva_fetched_at: @manifest_vva_fetched_at || Time.now.strftime('%D %l:%M%P %Z'),
      documents: docs
    }
  end
