...
3. Optional for setting up a machine, though you may work with these in the future \
    a. <https://github.com/department-of-veterans-affairs/caseflow-commons.git> \
    b. <https://github.com/department-of-veterans-affairs/caseflow-frontend-toolkit.git> \
    c. <https://github.com/department-of-veterans-affairs/caseflow-efolder.git> \
    d. <https://github.com/department-of-veterans-affairs/caseflow-facols.git> \
    e. <https://github.com/department-of-veterans-affairs/appeals-notebook.git>
...
1. Open terminal and run: \
    a. `brew install docker docker-compose colima` \
...
2. After downloading, click on one of the folders and follow the instructions in INSTALL_IC_README.txt to copy the libraries
...
4. In each of the extracted folders: \
    a. Navigate to the `/usr/local/homebrew/Cellar` subfolder \
    b. Copy the openssl folder to your local machine's `/usr/local/homebrew/Cellar` folder \
    c. If the folder `Cellar` in `/usr/local/homebrew` does not exist, create it with `mkdir /usr/local/homebrew/Cellar` \
...
1. Once your installation of all gems is complete, switch back to a standard MacOS terminal: \
    ...
    b. comment the line `export PATH=/usr/local/homebrew/bin:$PATH` \
    c. uncomment the line `export PATH=/opt/homebrew/bin:$PATH` \
    d. add the line `export PATH=$HOME/.nodenv/shims:$HOME/.rbenv/shims:$HOME/.pyenv/shims:$PATH` \
    e. comment the lines `eval "$({binary} init -)"` for rbenv, pyenv, and nodenv if applicable \
    f. if you added the line `eval $(/usr/local/homebrew/bin/brew shellenv)` after installing x86_64 homebrew, comment it out \
2. Open a terminal verify: \
	  a. that you are on arm64 by doing `arch` and checking the output \
	  b. that you are using arm64 brew by doing `which brew` and ensuring the output is `/opt/homebrew/bin/brew` \
...
7. run `make reset` to (re)create and seed the database; this takes a while (~45 minutes) \
	  a. if you get a database not found error, run `bundle exec rake db:drop db:create db:schema:load`, and then run `make reset` again \
8. open a second terminal tab/window \
9. run `make run-backend-m1` in one tab, and `make run-frontend` in the other \
	  a. the backend should launch on `localhost:3000`. go there in your browser to access Caseflow \
	  b. the frontend launches on `localhost:3500`; this is a server that enables hot-reloading of modules during development. going to this address in a browser will not work \
	  c. optionally, you can do `make run-m1` to launch both at the same time with Foreman, but if one server errors the other can remain running and lead to headache trying to get them back up
