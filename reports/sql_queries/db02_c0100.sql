...
/* RAILS_EQUIV
res=BvaDispatchTask.completed.where("closed_at > ?", Time.utc(2018, 10, 1, 4)).order(:closed_at).group_by{|t|
  [t.closed_at.beginning_of_month, t.appeal_type, t.appeal_id]
}.keys;
res2=res.group_by{|month, appeal_type, appeal_id| [month.strftime("%Y-%m-%d %H:%M:%S"), appeal_type] };
