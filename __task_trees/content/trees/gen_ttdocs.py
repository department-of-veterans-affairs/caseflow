...
def generate_docs(inputfile):
    ...
    for dockettype,basedir in dockettypes.items():
        ...
        extract_freq_child_parent(inputfile, basedir, dockettype)
...
childTaskFreq=100
childParentFreq=500
childParentPercent=0.4
def extract_freq_child_parent(inputfile, basedir, dockettype):
    cpList=[]
    for taskname,parentDict in parentlinksDict.items():
        if taskCountsDict[taskname]>=childTaskFreq:
            relationsCount=sum(parentDict.values())
            for parent,count in parentDict.items():
                if count>=childParentFreq or count/relationsCount >= childParentPercent:
                    percent=count/relationsCount
                    cpList.append({'child': taskname, 'parent': parent, 'count': count, 'percent': percent, 'label': "{:.0%}".format(percent)+f" ({count})"})

    with open(f'{basedir}/dot/freq-childparent.dot', 'w+') as dotf:
        dotf.write(gen_parentchild_graphviz("rankdir=TB;\nnode [shape=box];", cpList, 'parent', 'child', 'count'))

    gen_freq_childparent_md(basedir, dockettype, cpList)

def gen_freq_childparent_md(basedir, dockettype, cpList):
    with open(f'{basedir}/freq-childparent.md', 'w+') as pcf:
        pcf.write("---\n---\n<!-- DO NOT EDIT THIS FILE.  This file is autogenerated. -->\n")
        pcf.write(f'''| Frequent Child-Parent Relationships for [DR](../docket-DR/freq-childparent.md), [ES](../docket-ES/freq-childparent.md), [H](../docket-H/freq-childparent.md) |

# Frequent Child-Parent Relationships for '{dockettype}'

This page provide stats for: given a child task type, how frequent does the parent task type occur?

For this page, *frequent* is defined as (1) the child task type occurred at least {childTaskFreq} times and (2) either
* the child-parent relationship occurred at least {childParentFreq} times (**Count** column)
* or among all child-parent relationships with the same child task type, at least {round(childParentPercent*100)}% had the specific parent task type (**Percent** column)
...
| --------------- | ---------------- | ----- | ------- |
''')
        for edge in sorted(cpList, key=lambda kv: kv['count'], reverse=True):
            pcf.write(f"| [{edge['child']}]({edge['child']}.md) | [{edge['parent']}]({edge['parent']}.md) "+
                f"| {edge['count']} | {'{:.0%}'.format(edge['percent'])} |\n")
        pcf.write("{.parentChildTable}\n") # goldmark markdown to set CSS class on table
