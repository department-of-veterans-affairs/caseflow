      ...
      it "returns associations with other tables" do
        ...
        expect(subject.select_associations.map(&map_foreign_keys)).to match_array [
          ...
          [:versions, false, false,
           "item_id", "version_id", nil],
          # acts_as_tree declared in Task
          [:parent, true, false, "parent_id",
           "task_id", nil],
          [:children, false, false,
           "parent_id", "task_id", nil],
          # belongs_to declared in Task
          [:assigned_to, true, false,
           "assigned_to_id", "assigned_to_id", "assigned_to_type"],
          [:assigned_by, true, false,
           "assigned_by_id", "user_id", nil],
          [:cancelled_by, true, false,
           "cancelled_by_id", "user_id", nil],
          [:completed_by, true, false,
           "completed_by_id", "user_id", nil],
          [:appeal, true, false, "appeal_id",
           "appeal_id", "appeal_type"],
          # Polymorphic `belongs_to :appeal`-related associations
          [:ama_appeal, true, false,
           "appeal_id", "appeal_id", nil],
          [:legacy_appeal, true, false,
           "appeal_id", "legacy_appeal_id", nil],
          [:higher_level_review, true, false,
           "appeal_id", "higher_level_review_id", nil],
          [:supplemental_claim, true, false,
           "appeal_id", "supplemental_claim_id", nil],
          [:correspondence, true, false,
           "appeal_id", "correspondence_id", nil],
          # Polymorphic `belongs_to :appeal`-related STI associations
          [:remand, true, false, "appeal_id",
           "remand_id", nil],
          ...
          [:attorney_case_reviews, false,
           false, "task_id", "attorney_case_review_id", nil],
          [:task_timers, false, false,
           "task_id", "task_timer_id", nil],
          [:correspondences_appeals_task,
           false, true, "task_id", "correspondences_appeals_task_id", nil],
          [:correspondence_appeal, false,
           true, "correspondence_appeal_id", "correspondence_appeal_id", nil],
          # has_one declared in Task
          [:cached_appeal, false, true,
           "appeal_id", "cached_appeal_id", nil]
        ]
      end
    end
