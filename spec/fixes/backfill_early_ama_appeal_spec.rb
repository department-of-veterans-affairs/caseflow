...
##
# This RSpec provides an example of how to backfill (i.e., retroactively add missing data to) an AMA appeal.
#
# Example requests from the Board:
...
describe "Backfill early AMA appeal" do
  context "given Appeal with only a RootTask" do
    ...
    it "creates tasks and other records associated with a dispatched appeal" do
      ...
      # 2. Create BvaDispatchTask so that BVAAppealStatus reports a 'dispatched' status
      ...
      # Otherwise (i.e., if the user is known), set assigned_to and dates for the child task accordingly.
      org_dispatch_task.children.delete_all

      # Set org-assignee BvaDispatchTask attributes
      ...
      # 3. Create DecisionIssues for each RequestIssue -- see Bat-Team-Quick-Ref#split-decision-issues wiki page
      req_issues = appeal.request_issues.order(:id) # This is the same order as presented on the Case Details page
      expect(req_issues.pluck(:id)).to eq [2_000_000_024, 2_000_000_025, 2_000_000_026, 2_000_000_027]

      # Check the decision document for the appeal by going to Case Details, looking at when the case was dispatched,
      # then searching the veteran's Reader docs for a "BVA Decision" document with a similar date.
      # List all such docs with: `appeal.documents.where(type: "BVA Decision").pluck(:received_at, :type).sort`
      # Tip: download the PDF to copy and paste the decision description text for all issues.
      ...
      # 4. Create DecisionDocument for the appeal -- also see fixing_dispatched_appeal_spec.rb
