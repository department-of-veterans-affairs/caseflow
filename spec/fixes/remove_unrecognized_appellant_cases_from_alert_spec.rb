...
describe "Appeals with unrecognized appellants" do
  ...
  context "appeal2" do
    ...
    it "no longer shows these appeals in the AppealsWithNoTasksOrAllTasksOnHoldQuery" do
      ...
      dispatch_task = BvaDispatchTask.create_from_root_task(appeal2.root_task)
      user_dispatch_task = appeal2.tasks.where(type: "BvaDispatchTask").where(assigned_to_type: "User").first
      eng_task = EngineeringTask.create!(parent: user_dispatch_task, status: Constants.TASK_STATUSES.assigned, appeal: appeal2)
  ...
  context "appeal3" do
    ...
    it "no longer shows these appeals in the AppealsWithNoTasksOrAllTasksOnHoldQuery" do
      ...
      dispatch_task = BvaDispatchTask.create_from_root_task(appeal3.root_task)
      user_dispatch_task = appeal3.tasks.where(type: "BvaDispatchTask").where(assigned_to_type: "User").first
      eng_task = EngineeringTask.create!(parent: user_dispatch_task, status: Constants.TASK_STATUSES.assigned, appeal: appeal3)
  ...
  context "appeal4" do
    ...
    it "no longer shows these appeals in the AppealsWithNoTasksOrAllTasksOnHoldQuery" do
      ...
      dispatch_task = BvaDispatchTask.create_from_root_task(appeal4.root_task)
      user_dispatch_task = appeal4.tasks.where(type: "BvaDispatchTask").where(assigned_to_type: "User").first
      eng_task = EngineeringTask.create!(parent: user_dispatch_task, status: Constants.TASK_STATUSES.assigned, appeal: appeal4)
