...
describe Appeal, :all_dbs do
  ...
  context "#advance_on_docket?" do
    context "when a claimant is advance_on_docket?" do
      let(:appeal) do
        create(:appeal, claimants: [create(:claimant, :advance_on_docket_due_to_age)])
      end

      it "returns true" do
        expect(appeal.advance_on_docket?).to eq(true)
      end
    end

    context "when no claimant is advance_on_docket?" do
      let(:appeal) do
        create(:appeal)
      end

      it "returns false" do
        expect(appeal.advance_on_docket?).to eq(false)
      end
    end
  end
