...
describe Metric do
  ...
  describe "create_metric" do
    ...
    it "creates a javascript metric for performance" do
      ...
      expect(metric.metric_type).to eq(Metric::Caseflow::MetricAttributes::METRIC_TYPES[:performance])
    end
    ...
    it "creates a javascript metric for log" do
      ...
      expect(metric.valid?).to be true
      expect(metric.metric_type).to eq(Metric::Caseflow::MetricAttributes::METRIC_TYPES[:log])
    end
    ...
    it "creates a javascript metric for error" do
      ...
      expect(metric.valid?).to be true
      expect(metric.metric_type).to eq(Metric::Caseflow::MetricAttributes::METRIC_TYPES[:error])
    end
