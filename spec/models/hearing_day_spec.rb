...
describe HearingDay do
  ...
  context "Video Hearings returns video hearings that are not postponed or cancelled" do
    ...
    let!(:staff) { create(:staff, stafkey: "RO13", stc2: 2, stc3: 3, stc4: 4) }
    let!(:hearing) { create(:case_hearing, folder_nr: appeal.vacols_id) }
    let(:vacols_case2) do
      create(
        :case,
        bfregoff: "RO13",
    ...
    let(:appeal2) { create(:legacy_appeal, :with_veteran, vacols_case: vacols_case2) }
    let!(:hearing2) do
      create(
    ...
    context "When there are multiple hearings and multiple days" do
      let(:appeal_today) { create(:legacy_appeal, :with_veteran, vacols_case: create(:case)) }
      let!(:second_hearing_today) do
        create(:case_hearing, vdkey: hearing.vdkey, folder_nr: appeal_today.vacols_id)
      end
      let(:appeal_tomorrow) { create(:legacy_appeal, :with_veteran, vacols_case: create(:case)) }
