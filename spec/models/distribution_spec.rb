...
describe Distribution, :all_dbs do
  ...
  context "#batch_size" do
    it "is set to alternative batch size if judge has no attorneys" do
      expect(new_distribution.send(:batch_size)).to eq(CaseDistributionLever.find_by_item('alternative_batch_size').try(:value))
    end
    ...
    it "is set based on number of attorneys on team" do
      ...
      expect(new_distribution.send(:batch_size)).to eq(3 * CaseDistributionLever.find_by_item('batch_size_per_attorney').try(:value))
    end
  end
  ...
  context "requested distributions" do
    context "when priority_acd is enabled" do
      let(:limit) { 10 }
      let(:batch_size) { CaseDistributionLever.find_by_item('alternative_batch_size').try(:value) }
