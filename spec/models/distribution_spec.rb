...
describe Distribution, :all_dbs do
  ...
  context "#batch_size" do
    it "is set to alternative batch size if judge has no attorneys" do
      expect(new_distribution.send(:batch_size)).to eq(CaseDistributionLever.find_integer_lever(Constants.DISTRIBUTION.alternative_batch_size))
    end
    ...
    it "is set based on number of attorneys on team" do
      ...
      expect(new_distribution.send(:batch_size)).to eq(3 * CaseDistributionLever.find_integer_lever(Constants.DISTRIBUTION.batch_size_per_attorney))
    end
  end
  ...
  context "requested distributions" do
    context "when priority_acd is enabled" do
      let(:limit) { 10 }
      let(:batch_size) { CaseDistributionLever.find_integer_lever(Constants.DISTRIBUTION.alternative_batch_size) }
