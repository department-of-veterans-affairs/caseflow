...
RSpec.describe HearingsSupervisors, type: :model do
  describe ".perform" do
    it "is named correctly" do
      expect(described_class.singleton).to have_attributes(name: "Hearings Supervisors")
    end

    it "will only have one HearingsSupervisor no matter how many times it is run" do
      described_class.singleton
      described_class.singleton
      expect(Organization.where(name: "Hearings Supervisors").count).to eq(1)
    end
  ...
  describe "can_receive_task?" do
    ...
    let(:hearings_supervisors) { described_class.new }

    it "will always return false" do
      expect(hearings_supervisors.can_receive_task?).to eq(false)
    end
  end
end
