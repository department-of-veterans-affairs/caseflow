...
describe EstablishClaim do
  ...
  context ".actions_taken" do
    ...
    context "when complete" do
      ...
      let(:completion_status) { Task.completion_status_code(:completed) }
      ...
      context "when appeal is a Full Grant" do
        ...
        context "when an EP was established" do
          ...
          it { is_expected.to include("Established EP: 172BVAG - BVA Grant for Station 351 - Muskogee, OK") }
        end

        context "when a VBMS Note was added" do
        ...
        context "when processed via email" do
          ...
          let(:emailed_to_ro) { "RO84" }
          let(:email_recipient) { "appealcoach@va.gov" }
          let(:special_issues) { { radiation: true } }

          it { is_expected.to include("Sent email to: appealcoach@va.gov in Philadelphia COWAC - re: Radiation Issue(s)") }
        end
