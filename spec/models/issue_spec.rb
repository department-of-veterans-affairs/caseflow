...
# 250 compensation
# 250 pension
file_numbers.each_with_index do |file_number, index|
	case index
	when 0..499
		intake = HigherLevelReviewIntake.create!(veteran_file_number: file_number, user: RequestStore.store[:current_user])
	when 500..999
		intake = SupplementalClaimIntake.create!(veteran_file_number: file_number, user: RequestStore.store[:current_user])
	else
		puts "Unexpected index #{index} while creating intake"
		break
	end
	...
	case index
	when 0..249, 500..749
		review_params = {"receipt_date"=>"2023-06-29", "informal_conference"=>false, "same_office"=>nil, "benefit_type"=>"compensation", "legacy_opt_in_approved"=>false, "filed_by_va_gov"=>false, "veteran_is_not_claimant"=>false, "claimant_type"=>"Veteran"}
	when 250..499, 750..999
		review_params = {"receipt_date"=>"2023-06-29", "informal_conference"=>false, "same_office"=>nil, "benefit_type"=>"pension", "legacy_opt_in_approved"=>false, "filed_by_va_gov"=>false, "veteran_is_not_claimant"=>false, "claimant_type"=>"Veteran"}
	else
		puts "Unexpected index #{index} while assigning review params"
		break
	end

	permit_review_params = ActionController::Parameters.new(review_params)

	intake.review!(permit_review_params)

	case index
	when 0..249, 500..749
		issue_params = {"request_issues"=>[{"benefit_type"=>"compensation", "nonrating_issue_category"=>"Apportionment", "decision_text"=>"apportionment description", "decision_date"=>"2023-07-11", "ineligible_due_to_id"=>nil, "ineligible_reason"=>nil, "withdrawal_date"=>nil, "is_predocket_needed"=>nil}]}
	when 250..499, 750..999
		issue_params = {"request_issues"=>[{"benefit_type"=>"pension", "nonrating_issue_category"=>"Beneficiary Travel | Special Mode", "decision_text"=>"beneficiary travel description", "decision_date"=>"2023-07-11", "ineligible_due_to_id"=>nil, "ineligible_reason"=>nil, "withdrawal_date"=>nil, "is_predocket_needed"=>nil}]}
	else
			puts "Unexpected index #{index} while assigning issue params"
		break
	end

	permit_issue_params = ActionController::Parameters.new(issue_params)

	intake.complete!(permit_issue_params)
end
