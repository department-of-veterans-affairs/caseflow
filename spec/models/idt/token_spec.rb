...
describe IdtToken do
  before do
    Timecop.freeze(Time.utc(2019, 1, 1, 12, 0, 0))
  end

  let(:css_id) { "TEST_ID" }
  let(:key_token_pair) { IdtToken.generate_one_time_key_and_proposed_token }
  ...
  context "#activate_proposed_token" do
    it "successfully activates tokens with valid one time keys" do
      ...
      expect(IdtToken.activate_proposed_token(key, css_id)).to eq(true)
      expect(IdtToken.active?(token)).to eq(true)
    end

    it "raises an error when invalid one time key is used" do
      expect { IdtToken.activate_proposed_token(invalid_token, css_id) }
    ...
    it "raises on the second attempted use of a one time key" do
      ...
      expect { IdtToken.activate_proposed_token(key, css_id) }
        .to raise_error(Caseflow::Error::InvalidOneTimeKey)
    end
  end
  ...
  context "#active?" do
    it "returns true for valid tokens" do
      key, token = key_token_pair
      IdtToken.activate_proposed_token(key, css_id)
      expect(IdtToken.active?(token)).to eq(true)
    end

    it "returns false for invalid tokens" do
      expect(IdtToken.active?(invalid_token)).to eq(false)
    end
    ...
    it "returns false after a token expires" do
      ...
      IdtToken.activate_proposed_token(key, css_id)
      expect(IdtToken.active?(token)).to eq(true)
      IdtToken.client.expire("valid_tokens_key" + token, -1)
      expect(IdtToken.active?(token)).to eq(false)
    end
  end
end
