...
describe DecisionIssue, :postgres do
  ...
  describe "#create_contesting_request_issue!" do
    ...
    context "when rating_profile_date is blank" do
      it "sets contested_rating_issue_profile_date to nil" do
        request_issue = decision_issue.create_contesting_request_issue!(appeal)

        expect(request_issue.contested_rating_issue_profile_date).to be_nil
      end
    end
    ...
    context "when other attributes are present" do
      ...
      let(:decision_issue) do
        create(:decision_issue,
               rating_profile_date: rating_profile_date,
               diagnostic_code: "1234",
               description: "Test description",
               benefit_type: "compensation",
               caseflow_decision_date: "2024-07-28")
      end

      it "creates a RequestIssue with the correct attributes" do
        request_issue = decision_issue.create_contesting_request_issue!(appeal)

        expect(request_issue.contested_rating_issue_profile_date.to_s).to eq(rating_profile_date.utc.to_s)
        expect(request_issue.contested_rating_issue_diagnostic_code).to eq("1234")
        expect(request_issue.contested_issue_description).to eq("Test description")
