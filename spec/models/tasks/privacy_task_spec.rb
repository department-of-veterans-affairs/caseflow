...
describe PrivacyActTask do
  before do
    OrganizationsUser.add_user_to_organization(FactoryBot.create(:user), PrivacyTeam.singleton)
  end

  describe ".update!(status: Constants.TASK_STATUSES.cancelled)" do
    let(:root_task) { FactoryBot.create(:root_task) }
    let!(:p_task) { PrivacyActTask.create_from_root_task(root_task) }

    it "should create a task for the PrivacyTeam" do
      p_task.update!(status: Constants.TASK_STATUSES.cancelled)
      expect(p_task.reload.status).to eq(Constants.TASK_STATUSES.cancelled)
      expect(root_task.reload.children.select { |t| t.type == PrivacyActTask.name }.count).to eq(1)
    end
  end
  ...
  #   let(:root_task) { FactoryBot.create(:root_task) }
  #
  #   context "when PrivacyActTask is assigned to the Privacy Team organization" do
  #     let!(:p_task) { PrivacyActTask.create_from_root_task(root_task) }
  #
  ...
  #   end
  #
  #   context "when PrivacyActTask has been assigned to an individual" do
  ...
  #     let!(:qr_org_task) { PrivacyActTask.create_from_root_task(root_task) }
  ...
  #       }]
  #     end
  #     let!(:qr_person_task) { PrivacyActTask.create_many_from_params(qr_task_params, qr_user).first }
  #
  #     it "should create a task for BVA dispatch and close all PrivacyActTasks" do
  #       expect(root_task.children.select { |t| t.type == BvaDispatchTask.name }.count).to eq(0)
