  ...
  factory :case_hearing, class: VACOLS::CaseHearing do
    ...
    after(:build) do |hearing, evaluator|
      ...
      if hearing.vdkey.nil?
        regional_office = (hearing.hearing_type == HearingDay::REQUEST_TYPES[:central]) ? nil : "RO13"
        attrs = {
      ...
      if evaluator.user
        existing_staff_record = VACOLS::Staff.find_by_sdomainid(evaluator.user.css_id)
