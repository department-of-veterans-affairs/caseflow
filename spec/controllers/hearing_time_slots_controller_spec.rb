...
RSpec.describe HearingTimeSlotsController, :all_dbs, type: :controller do
  ...
  describe "#index" do
    context "with invalid params" do
      it "does not return anything" do
        ...
        expect(response_body["hearing_time_slots"]).to eq(nil)
      end
    end
    ...
    context "hearing day with multiple hearings" do
      ...
      it "returns correct data" do
        ...
        expect(response.status).to eq 200
        response_body = JSON.parse(response.body)
        expect(response_body["hearing_time_slots"].size).to eq 2
        expect(response_body["hearing_time_slots"].first.keys).to match_array(expected_keys)
        expect(response_body["hearing_time_slots"].second.keys).to match_array(expected_keys)
        expect(response_body["hearing_time_slots"].map { |res| res["hearing_time"] })
          .to match_array(["08:30", "12:00"])
        expect(response_body["hearing_time_slots"].map { |res| res["issue_count"] })
          .to match_array([ama_hearing.current_issue_count, legacy_hearing.current_issue_count])
        expect(response_body["hearing_time_slots"].map { |res| res["docket_number"] })
          .to match_array([ama_hearing.docket_number, legacy_hearing.docket_number])
        expect(response_body["hearing_time_slots"].map { |res| res["docket_name"] })
          .to match_array([ama_hearing.docket_name, legacy_hearing.docket_name])
        expect(response_body["hearing_time_slots"].map { |res| res["poa_name"] })
