...
  # Add the new tests for the PDF render time metrics logging
  describe "GET #show" do
    context "when feature toggles are enabled" do
      before do
        FeatureToggle.enable!(:pdf_page_render_time_in_ms)
        FeatureToggle.enable!(:metrics_monitoring)
      end

      it "logs metrics for PDF page render time" do
        get :show, params: { appeal_id: appeal.id }

        metric = Metric.last
        expect(metric).not_to be_nil
        expect(metric.metric_message).to eq("pdf_page_render_time_in_ms")
...
        FeatureToggle.disable!(:metrics_monitoring)
      end

      it "does not log metrics for PDF page render time" do
        get :show, params: { appeal_id: appeal.id }

        expect(Metric.last).to be_nil
      end
    end
  end
end
