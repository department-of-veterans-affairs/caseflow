    ...
    context "user is Global Admin" do
      ...
      context "there are hearings" do
        ...
        let!(:disposition_task1) do
          create(:assign_hearing_disposition_task, parent: hearing_task1, appeal: ama_hearing.appeal)
        end
        ...
        let!(:disposition_task2) do
          create(:assign_hearing_disposition_task, parent: hearing_task2, appeal: legacy_hearing1.appeal)
        end
        ...
        let!(:disposition_task3) do
          create(:assign_hearing_disposition_task, parent: hearing_task3, appeal: legacy_hearing2.appeal)
        end
      ...
      context "the after date parameters are passed" do
        ...
        it "sends the date to HearingsProfileHelper" do
          expect(HearingsProfileHelper).to receive(:profile_data).once.with(current_user, after: after).and_return({})
          get :index, params: { after_year: after.year, after_month: after.month, after_day: after.day }
        end
      end
      ...
      context "the limit parameter is passed" do
        it "sends the limit to HearingsProfileHelper" do
          expect(HearingsProfileHelper).to receive(:profile_data).once.with(current_user, limit: 100).and_return({})
