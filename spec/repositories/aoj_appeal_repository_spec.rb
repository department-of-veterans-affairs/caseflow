...
describe AojAppealRepository, :all_dbs do
  ...
  context ".build_appeal" do
    ...
    subject { AojAppealRepository.build_appeal(case_record) }
  ...
  context ".load_vacols_data" do
    ...
    subject { AojAppealRepository.load_vacols_data(appeal) }
    it do
      expect(AojAppealRepository).to receive(:set_vacols_values).exactly(1).times
      is_expected.to eq(true)
    end
  end

  context ".normalize_vacols_date" do
    subject { AojAppealRepository.normalize_vacols_date(datetime) }
  ...
  context ".ssoc_dates_from" do
    subject { AojAppealRepository.ssoc_dates_from(case_record) }
    it do
  ...
  context ".folder_type_from" do
    subject { AojAppealRepository.folder_type_from(folder_record) }
  ...
  context "#location_after_dispatch" do
    ...
    subject { AojAppealRepository.location_after_dispatch(appeal: appeal) }
  ...
  describe ".find_case_record" do
    subject { AojAppealRepository.find_case_record(ids, ignore_misses: ignore_misses) }
  ...
  context "AMA Opt-ins and rollbacks on previously decided appeals" do
    ...
    describe ".opt_in_decided_appeal!" do
      subject { AojAppealRepository.opt_in_decided_appeal!(appeal: appeal, user: user, closed_on: new_decision_date) }
    ...
    describe ".rollback_opt_in_on_decided_appeal!" do
      subject do
        AojAppealRepository.rollback_opt_in_on_decided_appeal!(
          appeal: appeal,
          user: user,
