    ...
    it "returns daily stats" do
      expect(subject.collect_stats).to match_array(
        [
          ...
          { :metric => "daily_counts.totals.req_issue", :value => 2, "req_issue" => "higherlevelreview" },
          { :metric => "daily_counts.totals.claim_establishment", :value => 1, "claim_establishment" => "nil" },
          { :metric => "daily_counts.totals.dispatch", :value => 1, "dispatch" => "establishclaim.unassigned" },
