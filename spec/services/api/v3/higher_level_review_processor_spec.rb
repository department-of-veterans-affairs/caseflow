...
describe Api::V3::HigherLevelReviewProcessor, :all_dbs do
  ...
  context "::CATEGORIES_BY_BENEFIT_TYPE" do
    ...
    it("has string keys") do
      expect(subject.keys.all? { |key| key.is_a?(String) }).to be(true)
    end
    it("has array values") do
      expect(subject.values.all? { |val| val.is_a?(Array) }).to be(true)
    end
    it("and each array is a non-empty array of strings") do
      expect(
        subject.values.all? do |array|
          array.any? && array.all? { |element| element.is_a?(String) }
        end
      ).to be(true)
    end
  end
