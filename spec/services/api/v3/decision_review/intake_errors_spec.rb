...
context Api::V3::DecisionReview::IntakeErrors do
  context ".render_hash" do
    let(:intake_start_failed_intake_error) do # status 422
      Api::V3::DecisionReview::IntakeError.new(:intake_start_failed)
    end

    let(:veteran_not_accessible_intake_error) do # status 403
      Api::V3::DecisionReview::IntakeError.new(:veteran_not_accessible)
    end

    let(:unknown_intake_error) { Api::V3::DecisionReview::IntakeError.new } # status 500
    ...
    it "should return a properly formatted render hash with a status 422 (the highest one)" do
      expect(
        Api::V3::DecisionReview::IntakeErrors.new(
