  ...
  describe ".process" do
    context "when intake is not HigherLevelReview" do
      it "creates a new supplemental claim" do
        ...
        expect(described_class.process!(event: event, parser: parser)).to eq(EventRecord.last)
      end
    end
    ...
    context "when an error occurs" do
      it "raises DecisionReviewCreatedCreateClaimReviewError" do
        allow(HigherLevelReview).to receive(:create).and_raise(Caseflow::Error::DecisionReviewCreatedCreateClaimReviewError, "Error message")
