...
RSpec.feature "Change hearing disposition", :all_dbs do
  ...
  let(:case_hearing) { create(:case_hearing, vdkey: hearing_day.id, folder_nr: appeal.vacols_id) }
  let(:hearing) do
  ...
  context "there's a hearings management user" do
    ...
    context "a hearing has mistakenly been marked postponed" do
      ...
      let(:case_hearing) do
        create(:case_hearing, :disposition_postponed, vdkey: hearing_day.id, folder_nr: appeal.vacols_id)
      end
