...
feature "Appeal Edit issues", :all_dbs do
  ...
  context "User is a member of the Supervisory Senior Council" do
    ...
    let!(:request_issue_1) { create(:request_issue, id: 22, decision_review: appeal2, decision_date: profile_date, 
      contested_rating_issue_reference_id: "def456", contested_rating_issue_profile_date: profile_date,
      contested_issue_description: "PTSD denied", contention_reference_id: "3897", benefit_type: "Education") }
    
    let!(:request_issue_2) { create(:request_issue, id: 25, decision_review: appeal2, decision_date: profile_date, 
      contested_rating_issue_reference_id: "blah1234", contested_rating_issue_profile_date: profile_date,
      contested_issue_description: "PTSD denied", contention_reference_id: "78910", benefit_type: "Education") }
    let!(:appeal_serializer) { WorkQueue::AppealSerializer.new(appeal2, params: { user: current_user }).serializable_hash }
    scenario "less than 2 request issues on the appeal, the split appeal button doesn't show" do
      ...
      expect(current_user.organizations.find_by(name: "Supervisory Senior Council")) == SupervisorySeniorCouncil
      expect(appeal2.decision_issues.length + appeal2.request_issues.length).to be < 2
      expect(page).to_not have_button("split-appeal")
    end

    scenario "2 or more request issues, the split appeal button shows " do
