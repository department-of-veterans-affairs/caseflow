  ...
  describe "Hearing Postponement Request Mail Task" do
    ...
    shared_examples_for "scheduling a hearing" do
      ...
      it "sends proper notifications", skip: "test is failing in local env and github actions" do
        scheduled_payload = AppellantNotification.create_payload(appeal, Constants.EVENT_TYPE_FILTERS.hearing_scheduled).to_json
        if appeal.hearings.any?
          postpone_payload = AppellantNotification.create_payload(appeal, Constants.EVENT_TYPE_FILTERS.postponement_of_hearing)
            .to_json
      ...
      shared_context "async actions" do
        context "async actions" do
          it "sends withdrawal of hearing notification" do
            withdrawal_payload = AppellantNotification.create_payload(appeal, Constants.EVENT_TYPE_FILTERS.withdrawal_of_hearing).to_json
