    ...
    context "when user is in Bva organization" do
      ...
      context 'when acd_exclude_from_affinity toggle is off' do
        ...
        scenario "user can view and change Exclude from Affinity Appeals toggle" do
          visit("/team_management")
          expect(page).to have_content("Judge Teams")
          expect(page).not_to have_content("*When the box is checked, the judge will not receive appeals with which there is an existing affinity relationship. Any appeal with an affinity relationship to that judge will immediately be released for distribution to any judge once the appeal is ready to distribute. Appeals that are tied (e.g., legacy hearing) are unaffected by this value.") # rubocop:disable Layout/LineLength
