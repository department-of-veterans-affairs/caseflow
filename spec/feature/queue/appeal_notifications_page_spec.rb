...
RSpec.feature "Notifications View" do
  ...
  shared_examples "with notifications" do
    ...
    it "table can filter by each column, and filter by multiple columns at once" do
      ...
      expect(cells[6]).to have_content("Appeal docketed")
      ...
      expect(cells[38]).to have_content("Email")
      ...
      expect(cells[15]).to have_content("example@example.com")
      ...
      expect(cells[22]).to have_content("Delivered")
    ...
    it "notification page response button renders and behaves properly" do
      visit appeal_notifications_page
      expect(page).to have_current_path(appeal_notifications_page)
      page.find("svg", class: "PlusIcon", match: :first).click
      expect(page).to have_selector("tr", class: "responseDetailsRow")
      page.find("svg", class: "MinusIcon", match: :first).click
      expect(page).not_to have_selector("tr", class: "responseDetailsRow")
    end
  end
