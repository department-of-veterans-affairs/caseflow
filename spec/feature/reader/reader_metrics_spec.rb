...
RSpec.feature "Reader", :all_dbs, type: :feature do
  ...
  let(:metric_get_pdf_page) { "Getting PDF page #{page_number} from PDFJS document" }
  let(:metric_store_pdf_page) { "Storing PDF page #{page_number} text in Redux" }
  let(:metric_render_pdf_page) { "Rendering PDF page #{page_number} text" }
  let(:metric_render_pdf_time) { "PDF render time in Milliseconds" }
  ...
  context "Capture the 5 Reader Metrics for one page document" do
    ...
    scenario "capture each metric once" do
      ...
      nil_in_db(metric_render_pdf_time)
      nil_in_db(metric_get_pdf_page)
      nil_in_db(metric_store_pdf_page)
      nil_in_db(metric_render_pdf_page)

      visit "/reader/appeal/#{appeal.vacols_id}/documents/#{documents[0].id}"
      sleep 5

      exists_in_db(metric_get_pdf_doc)
      exists_in_db(metric_render_pdf_time)
      exists_in_db(metric_get_pdf_page)
      exists_in_db(metric_store_pdf_page)
      exists_in_db(metric_render_pdf_page)

      event_id = Metric.where(metric_message: metric_render_pdf_time).last.event_id
  ...
  context "Capture the 5 Reader Metrics for two page document" do
    ...
    scenario "capture 'Getting PDF document' and 'pdf_page_render_time_in_ms' metrics once and other metrics gets called per overscan value" do
      nil_in_db(metric_get_pdf_doc)
      nil_in_db(metric_render_pdf_time)
      nil_in_db(metric_get_pdf_page)
      nil_in_db(metric_store_pdf_page)
      ...
      metric_attributes = Metric.where(metric_message: metric_render_pdf_page).last.metric_attributes
      num_pages = metric_attributes["numPagesInDoc"]
      overscan = metric_attributes["overscan"]

      exists_in_db(metric_get_pdf_doc)
      exists_in_db(metric_render_pdf_time)

      num_metric_calls = num_pages >= overscan ? overscan : num_pages

      num_metric_calls.each do |x|
        let(:page_number) { x }
        exists_in_db(metric_get_pdf_page)
        exists_in_db(metric_store_pdf_page)
        exists_in_db(metric_render_pdf_page)
