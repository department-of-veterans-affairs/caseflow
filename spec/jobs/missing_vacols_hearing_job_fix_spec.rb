...
describe MissingVacolsHearingJobFix do
  ...
  context "missing_vacols_hearing" do
    ...
    context "when one of the AssignHearingDispositionTasks do not have an associated hearing" do
      it "continues with the rest of the tasks and logs the error" do
        ...
        expect(task3.status).to eq(Constants.TASK_STATUSES.assigned)
        expect(Rails.logger).to receive(:error).with("ALERT------- Task Id's: #{task2.id} " \
        "are missing associated hearings.This requires manual remediation------- ALERT")
