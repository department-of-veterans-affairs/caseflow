...
RSpec.describe PersonAndVeteranEventRemediationJob, type: :job do
  ...
  # # Create event records using the factory with traits
  # let!(:person_event_record) { create(:event_record, :person) }
  # let!(:veteran_event_record) { create(:event_record, :veteran) }

  # Mocking external services and data
  let(:event_record_person) { instance_double("EventRecord", evented_record_id: 1, evented_record_type: "Person", evented_record: person) }
  let(:event_record_veteran) { instance_double("EventRecord", evented_record_id: 2, evented_record_type: "Veteran", info: event_info) }

  let(:person) { instance_double("Person", ssn: "123-45-6789") }
  ...
  let(:duplicate_person_service) { instance_double("DuplicatePersonRemediationService") }
  let(:veteran_remediation_service) { instance_double("VeteranRecordRemedationService") }
  ...
  describe "#perform" do
    context "when processing Person events" do
      before do
        ...
        allow(DuplicatePersonRemediationService).to receive(:new).and_return(duplicate_person_service)
    ...
    context "when processing Veteran events" do
      before do
        ...
        allow(VeteranRecordRemedationService).to receive(:new).and_return(veteran_remediation_service)
