...
describe NationalHearingQueueEntryRefreshJob, :postgres do
  ...
  describe "#perform" do
    ...
    context "when the queue refresh is expecting to timeout" do
      ...
      before(:each) do
        allow(NationalHearingQueueEntry).to receive(:refresh).and_raise(ActiveRecord::QueryCanceled)
      end
      ...
      it "calls timeout_set twice" do
        expect(job).to receive(:timeout_set).exactly(2).times.and_call_original
        job.perform
      end
