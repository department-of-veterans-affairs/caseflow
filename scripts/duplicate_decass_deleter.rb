...
records = VACOLS::Decass.select("defolder").
  where("deadtim >= ?", Date.new(2018, 8, 16)).
  group("defolder").having("count(*) > 1")
defolders = records.map { |r| r.defolder }
...
defolders.each do |defolder|
  ...
  VACOLS::Decass.transaction do
    ...
    records_with_json_rep.each_pair do |k, records|
      if records.length == 1
        next
      end
      query_select = <<EOS
select 1
from decass
where #{predicate_from_record(records[0], records.length - 1)}
EOS
      num_found = 0
      ...
      fail unless num_found == records.length - 1

      puts "Deleting #{records.length - 1} duplicates of #{k}"
      query_delete = <<EOS
delete
from decass
where #{predicate_from_record(records[0], records.length - 1)}
