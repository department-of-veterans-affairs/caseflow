...
BEGIN
    ...
    into litcnt
    from cova
    where cvfolder = folder1 and cvissseq is not null and cvdisp in ('2', '4', '8');
    if litcnt > 0 then
        return '2';
    end if;
...
CREATE OR REPLACE FUNCTION "VACOLS_DEV"."CHARGED_BY"(folder CHAR, loc CHAR)
    ...
    CURSOR DV_Cur is SELECT LOCSTOUT
                     FROM PRIORLOC
                     WHERE LOCKEY = folder
                       AND LOCSTTO = loc
...
CREATE OR REPLACE FUNCTION "VACOLS_DEV"."HEARING_DATE_VLJ"(appealid CHAR, docket CHAR, vlj CHAR)
    RETURN DATE as
    hrdate DATE;
    CURSOR DN_Cur is
        SELECT HEARING_DATE
...
CREATE OR REPLACE FUNCTION "VACOLS_DEV"."HEARING_PENDING_ADV"(folder CHAR, inhr CHAR, inha CHAR)
    RETURN CHAR AS
    pending varchar2(1);
    hrcnt   number;
...
CREATE OR REPLACE FUNCTION "VACOLS_DEV"."MAILACT_PREV"(folder CHAR, actdate DATE)
    RETURN DATE AS
    prevdate DATE;
    CURSOR DV_Cur is SELECT macompdate
                     FROM MAILACT
...
CREATE OR REPLACE FUNCTION "VACOLS_DEV"."POST_HRG_DEC"(appealid CHAR, docket CHAR, hrgdate DATE)
    RETURN DATE AS
    decdate DATE;
    CURSOR DV_Cur is SELECT bfddec
                     FROM BRIEFF,
                          FOLDER
                     WHERE TICKNUM = BFKEY
                       and (TITRNUM = appealid AND (tinum = docket or tinum is null))
...
CREATE OR REPLACE FUNCTION "VACOLS_DEV"."PREV_BVA_DEC"(appealid char, decdate Date)
    RETURN NUMBER AS
    deccnt number;
...
CREATE OR REPLACE FUNCTION "VACOLS_DEV"."PRIORDEC_MEMID"(in_ac CHAR, in_vlj CHAR, in_dpdcn DATE,
    ...
    hrdate date;
    mdate  date;
    CURSOR Hr_Cur is SELECT BOARD_MEMBER, HEARING_DATE
                     from HEARSCHED,
                          FOLDER
                     where TICKNUM = FOLDER_NR
                     ...
                     order by HEARING_DATE DESC ;
    CURSOR Brf_Cur is SELECT BFMEMID
                      from BRIEFF
...
CREATE OR REPLACE FUNCTION "VACOLS_DEV"."PRIORLOC_55_OR_81"(folder CHAR, locdate DATE)
    RETURN CHAR AS
    prevloc varchar2(16);
    CURSOR DV_Cur is SELECT LOCSTTO
                     FROM PRIORLOC
                     WHERE LOCKEY = folder
                       AND locdout < locdate
...
CREATE OR REPLACE FUNCTION "VACOLS_DEV"."PRIORLOC_81_DATE"(folder CHAR)
    RETURN DATE AS
    prevdate DATE;
    CURSOR DV_Cur is SELECT LOCDOUT
                     FROM PRIORLOC
                     WHERE LOCKEY = folder
                       AND locstto = '81'
                     order by LOCDOUT DESC;
...
CREATE OR REPLACE FUNCTION "VACOLS_DEV"."PRIORLOC_CNT_ADM"(folder CHAR, locdate1 DATE, locdate2 DATE)
    RETURN NUMBER AS
    prcnt number;
...
CREATE OR REPLACE FUNCTION "VACOLS_DEV"."PRIORLOC_CNT_AFTER"(folder CHAR, locdate DATE)
    RETURN NUMBER AS
    prcnt number;
...
CREATE OR REPLACE FUNCTION "VACOLS_DEV"."TRANS_ATTACHDATE"(folder CHAR)
    ...
    prevdate DATE;
    atdate   date;
    CURSOR AT_Cur is select imgadtm
                     from attach
                     where imgtkky = folder
                       and imgdoctp = 'CT'
                     order by imgadtm DESC;
