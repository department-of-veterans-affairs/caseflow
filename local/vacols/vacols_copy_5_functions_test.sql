...
BEGIN
    if
        (issue_code = '09' or issue_code = '31') then
        appeal_cat := 'CH DIS';
    elsif
        ((TO_NUMBER(issue_code) >= 1 and TO_NUMBER(issue_code) <= 8)
        ...
        (TO_NUMBER(issue_code) >= 36 and TO_NUMBER(issue_code) <= 45) then
        appeal_cat := 'DTH CMP';
    elsif
        (TO_NUMBER(issue_code) >= 50 and TO_NUMBER(issue_code) <= 56) then
        appeal_cat := 'DTH PNS';
    elsif
        (issue_code = '77'
            or issue_code = '78'
        ...
        ((TO_NUMBER(issue_code) >= 60 and TO_NUMBER(issue_code) <= 65)
        ...
        (TO_NUMBER(issue_code) >= 66 and TO_NUMBER(issue_code) <= 70) then
        appeal_cat := 'VR+E';
    elsif
        (TO_NUMBER(issue_code) >= 71 and TO_NUMBER(issue_code) <= 75) then
        appeal_cat := 'WOEA';
    elsif
        issue_code = '76' then
        appeal_cat := 'WAIVER';
...
BEGIN
    ...
    if
        days > 0 and days <= 150 then
        df := 'A 0-150';
    elsif
        days > 150 and days <= 250 then
        df := 'B 151-250';
    elsif
        days > 250 and days <= 350 then
        df := 'C 251-350';
    elsif
        days > 350 and days <= 450 then
        df := 'D 351-450';
    elsif
        days > 450 and days <= 550 then
        df := 'E 451-550';
    elsif
        days > 550 and days <= 650 then
        df := 'F 551-650';
    elsif
        days > 650 and days <= 750 then
        df := 'G 651-750';
    elsif
        days > 750 and days <= 850 then
        df := 'H 751-850';
    else
        df := 'I Over 850';
    end if;
...
BEGIN
    ...
    where isskey = folder
      and issprog = '02'
      and isscode in ('09', '12', '17')
      and (issdc between '1' and '9' or issdc is null);

    select distinct to_number(clmfld), to_number(efolder)
    into vols, efolders
    from othdocs
    where ticknum = folder;
    if
        efolders > 0 then
        doccnt := efolders / 100;
    elsif
...
BEGIN
    ...
    adesc
        := to_char(duedate, 'mm/dd/yy') || ' ' || acode || ' ' || adesc;
...
BEGIN
    if
        substr(docket, 1, 2) < '89' and substr(docket, 1, 2) > '50' then
    ...
    if
        (substr(dockyymm, 4, 2)) between '00' and '50' then
...
BEGIN
    ...
    if
        to_char(indate, 'MM') >= '01' and to_char(indate, 'MM') <= '09' then
...
CREATE OR REPLACE FUNCTION "VACOLS_TEST"."HEARING_CLARIFICATION"(appealid CHAR, docket CHAR)
    ...
    CURSOR DN_Cur is
        ...
        FROM ATTACH,
             FOLDER
        WHERE TICKNUM = IMGTKKY
          and (TITRNUM = appealid AND (tinum = docket or tinum is null))
          and IMGDOCTP = 'HR'
        order by IMGADTM DESC;
...
CREATE OR REPLACE FUNCTION "VACOLS_TEST"."HEARING_DATE_VLJ"(appealid CHAR, docket CHAR, vlj CHAR)
    RETURN DATE as
    hrdate DATE;
    CURSOR DN_Cur is
        SELECT HEARING_DATE
...
CREATE OR REPLACE FUNCTION "VACOLS_TEST"."HEARING_DISP"(appealid CHAR)
    ...
    hrinfo varchar2(20);
    CURSOR DV_Cur is
        select hearing_disp || ' ' || to_char(hearing_date, 'mm/dd/yy')
                   || ' ' || nvl(aod, 'N') || ' ' || to_char(holddays, '999')
        into hrinfo
        from BRIEFF,
             HEARSCHED
        where bfkey = folder_nr
          and bfcorlid = appealid
          and hearing_type in ('C', 'T', 'V', 'R')
        order by hearing_date Desc;
...
BEGIN
    select max(to_char(holddays, '999'))
    into hrinfo
    from hearsched
    where folder_nr = folder
      and hearing_type in ('C', 'T', 'V', 'R')
...
CREATE OR REPLACE FUNCTION "VACOLS_TEST"."HEARING_PENDING_ADV"(folder CHAR, inhr CHAR, inha CHAR)
    RETURN CHAR AS
    pending varchar2(1);
    hrcnt
            number;
...
BEGIN
    ...
    if
        inha = '1' or inha = '2' or inha = '6' then
        return 'N';
    end if;
...
BEGIN
    select hearing_type, to_char(hearing_date, 'mm/dd/yy')
    into hrtype, hrdate
    from hearsched
    where folder_nr = folder
      and hearing_type in ('C', 'T', 'V', 'R')
...
BEGIN
    -- Travel Board
    if
        substr(vdkey, 5, 1) = '-' then

        if substr(vdkey, 7, 1) = '-' then
            trip := to_number(substr(vdkey, 6, 1));
            leg
                := to_number(substr(vdkey, 8, 1));
        elsif
            substr(vdkey, 8, 1) = '-' then
            trip := to_number(substr(vdkey, 6, 2));
            leg
                := to_number(substr(vdkey, 9, 1));
        else
            trip := to_number(substr(vdkey, 6, 3));
            leg
                := to_number(substr(vdkey, 10, 1));
        end if;

        select tbro
        into hrro
        from tbsched
        where tbyear = substr(vdkey, 1, 4)
          and tbtrip = trip
          and tbleg = leg;
...
CREATE OR REPLACE FUNCTION "VACOLS_TEST"."HEARING_VLJ"(appealid CHAR, docket CHAR)
    ...
    team
           varchar2(10);
    CURSOR DN_Cur is
        SELECT HEARING_TYPE, BOARD_MEMBER
        FROM HEARSCHED,
             FOLDER
        WHERE TICKNUM = FOLDER_NR
          and (TITRNUM = appealid AND (tinum = docket or tinum is null))
...
BEGIN
    ...
    from assign
    where tsktknm = folder
      and substr(tskactcd, 1, 1) in ('1', '2', '3');
    RETURN imecnt;
END;
/
...
BEGIN
    if
        (ascii(zip_code) >= 48 and ascii(zip_code) <= 57) then
        isdigitfn := 'TRUE';
    else
        isdigitfn := 'FALSE';
    end if;
...
CREATE OR REPLACE FUNCTION "VACOLS_TEST"."MAILACT_PREV"(folder CHAR, actdate DATE)
    ...
    CURSOR DV_Cur is
        SELECT macompdate
        FROM MAILACT
        WHERE mafolder = folder
          AND macompdate < actdate
        order by macompdate DESC;
...
BEGIN
    ...
    into otcnt
    from decass
    where defolder = folder
      and substr(deprod, 1, 2) = 'OT';
...
CREATE OR REPLACE FUNCTION "VACOLS_TEST"."PREV_ATTY"(appealid CHAR, docket CHAR)
    ...
    CURSOR DN_Cur is
        ...
        WHERE TICKNUM = BFKEY
          and (TITRNUM = appealid AND (tinum = docket or tinum is null))
          and (BFATTID is not null and BFATTID <> '000')
...
CREATE OR REPLACE FUNCTION "VACOLS_TEST"."PREV_BVA_DEC"(appealid char, decdate Date)
    RETURN NUMBER AS
    deccnt number;
...
BEGIN
    ...
    if
        memid <> '000' and memid <> '999' and memid is not null then
        return memid ;
    end if;
...
CREATE OR REPLACE FUNCTION "VACOLS_TEST"."PREV_VLJ_RETIRED"(appealid CHAR, docket CHAR)
    ...
    CURSOR BF_Cur is
        ...
        WHERE TICKNUM = BFKEY
          and (TITRNUM = appealid AND (tinum = docket or tinum is null))
          and (BFATTID is not null and BFMEMID <> '000')
        order by BFDDEC DESC;
...
BEGIN
    ...
    from hearsched
    where folder_nr = folder
      and hearing_type in ('C', 'V', 'R')
      and hearing_date is not null
      and board_member is not null
      and hearing_disp is null;
    RETURN hrcnt;
END;
/
...
CREATE OR REPLACE FUNCTION "VACOLS_TEST"."PRIORDEC_MEMID"(in_ac CHAR, in_vlj CHAR, in_dpdcn DATE,
    ...
    mdate
          date;
    CURSOR Hr_Cur is
        SELECT BOARD_MEMBER, HEARING_DATE
        from HEARSCHED,
             FOLDER
        where TICKNUM = FOLDER_NR
          and TITRNUM = in_appealid
          AND tinum = in_tinum
          and HEARING_DISP = 'H'
          AND HEARING_TYPE in ('C', 'T', 'V', 'R')
          and (HEARING_DATE >= in_dpdcn or in_dpdcn is null)
        order by HEARING_DATE DESC;
    CURSOR Brf_Cur is
        SELECT BFMEMID
        from BRIEFF
        where BFCORLID = in_appealid
          and BFDDEC = in_dpdcn
          and BFMEMID > '000'
        order by BFDDEC DESC;
...
BEGIN
    if
        in_ac <> '3' and in_ac <> '7' and in_vlj <> '000' then
        memid := in_vlj;
        return memid;
    end if;
    ...
    if
        memid is null or memid = '000' then
        memid := '';
    end if;
...
BEGIN
    sodate
        := nvl(vsodate, to_date('01-jan-1995'));
...
CREATE OR REPLACE FUNCTION "VACOLS_TEST"."PRIORLOC_CNT_ADM"(folder CHAR, locdate1 DATE, locdate2 DATE)
    RETURN NUMBER AS
    prcnt number;
...
BEGIN
    ...
    from PRIORLOC
    where lockey = folder
      and locdout between locdate1 and locdate2
      and locstto in ('24', '45', '55', '59', '11', '12', '26', '14', '57');
...
CREATE OR REPLACE FUNCTION "VACOLS_TEST"."PRIORLOC_CNT_AFTER"(folder CHAR, locdate DATE)
    RETURN NUMBER AS
    prcnt number;
...
BEGIN
    if
        (represent_code = 'A') then
        rep_code := 'American Legion';
    elsif
        (represent_code = 'B') then
        rep_code := 'AmVets';
    elsif
        (represent_code = 'C') then
        rep_code := 'ARC';
    elsif
        (represent_code = 'D') then
        rep_code := 'DAV';
    elsif
        (represent_code = 'E') then
        rep_code := 'JWV';
    elsif
        (represent_code = 'F') then
        rep_code := 'MOPH';
    elsif
        (represent_code = 'G') then
        rep_code := 'PVA';
    elsif
        (represent_code = 'H') then
        rep_code := 'VFW';
    elsif
        (represent_code = 'I') then
        rep_code := 'State Svc Org';
    elsif
        (represent_code = 'J' or represent_code = 'K' or
        ...
        (represent_code = 'L') then
        rep_code := 'None';
    elsif
        (represent_code = 'T') then
        rep_code := 'Attorney';
    elsif
        (represent_code = 'U') then
        rep_code := 'Agent';
    elsif
        (represent_code = 'V') then
        rep_code := 'VVA';
    end if;
    RETURN rep_code;
END rep_code;
...
BEGIN
    ...
    if
        in_status = 'REM' and in_ac = '1' then
        return 'R';
    end if;
