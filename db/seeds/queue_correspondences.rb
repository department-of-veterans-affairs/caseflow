...
module Seeds
  ...
  class QueueCorrespondences < Base
    ...
    def create_queue_correspondences
      ...
      20.times do
        corres = create_correspondence_with_review_package_and_failed_upload_task(appeal)
      end
      ...
      20.times do
        corres = create_correspondence_with_action_required_tasks(appeal)
      end

      # 10 Correspondences with in-progress CorrespondenceRootTask and completed Mail Task
      10.times do
        corres = create_correspondence_with_in_progress_root_task_and_completed_mail_task(appeal)
      end
      ...
      mail_task_pid_for_cavc = pending_tab_cavc_mailtask(create_correspondence(appeal), status: "assigned")
      20.times do
        corres = create_pending_tasks(mail_task_pid_for_cavc, appeal)
      end

      mail_task_pid_for_congressInterest = pending_tab_congress_interest_mailtask(create_correspondence(appeal), status: "assigned")
      20.times do
