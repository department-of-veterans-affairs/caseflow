...
ActiveRecord::Schema.define(version: 2023_03_08_235904) do
  ...
  create_view "business_line_tasks", materialized: true, sql_definition: <<-SQL
      SELECT tasks.id,
      tasks.appeal_id,
      tasks.appeal_type,
      tasks.assigned_at,
      ...
      tasks.cancelled_by_id,
      tasks.closed_at,
      tasks.created_at,
      tasks.instructions,
      tasks.parent_id,
      tasks.placed_on_hold_at,
      tasks.started_at,
      tasks.status,
      tasks.type,
      tasks.updated_at,
      ...
      tasks.veteran_ssn
     FROM ( SELECT tasks_1.id,
              tasks_1.appeal_id,
              tasks_1.appeal_type,
              tasks_1.assigned_at,
              tasks_1.assigned_by_id,
              tasks_1.assigned_to_id,
              tasks_1.assigned_to_type,
              tasks_1.cancellation_reason,
              tasks_1.cancelled_by_id,
              tasks_1.closed_at,
              tasks_1.created_at,
              tasks_1.instructions,
              tasks_1.parent_id,
              tasks_1.placed_on_hold_at,
              tasks_1.started_at,
              tasks_1.status,
              tasks_1.type,
              tasks_1.updated_at,
              ...
              COALESCE(NULLIF(
                  CASE
                      WHEN higher_level_reviews.veteran_is_not_claimant THEN COALESCE(NULLIF(concat(unrecognized_party_details.name, ' ', unrecognized_party_details.last_name), ' '::text), NULLIF(concat(people.first_name, ' ', people.last_name), ' '::text), (bgs_attorneys.name)::text)
                      ELSE concat(veterans.first_name, ' ', veterans.last_name)
                  END, ' '::text), 'claimant'::text) AS claimant_name,
              veterans.participant_id AS veteran_participant_id,
              veterans.ssn AS veteran_ssn
             FROM ((((((((tasks tasks_1
               JOIN higher_level_reviews ON (((higher_level_reviews.id = tasks_1.appeal_id) AND ((tasks_1.appeal_type)::text = 'HigherLevelReview'::text))))
               JOIN request_issues ON (((request_issues.decision_review_id = higher_level_reviews.id) AND ((request_issues.decision_review_type)::text = 'HigherLevelReview'::text))))
               JOIN veterans ON (((veterans.file_number)::text = (higher_level_reviews.veteran_file_number)::text)))
               LEFT JOIN claimants ON (((claimants.decision_review_id = tasks_1.appeal_id) AND ((claimants.decision_review_type)::text = (tasks_1.appeal_type)::text))))
               LEFT JOIN people ON (((claimants.participant_id)::text = (people.participant_id)::text)))
               ...
               LEFT JOIN bgs_attorneys ON (((claimants.participant_id)::text = (bgs_attorneys.participant_id)::text)))
            GROUP BY tasks_1.id, veterans.participant_id, veterans.ssn, veterans.first_name, veterans.last_name, unrecognized_party_details.name, unrecognized_party_details.last_name, people.first_name, people.last_name, higher_level_reviews.veteran_is_not_claimant, bgs_attorneys.name
          UNION ALL
           SELECT tasks_1.id,
              tasks_1.appeal_id,
              tasks_1.appeal_type,
              ...
              COALESCE(NULLIF(
                  CASE
                      WHEN supplemental_claims.veteran_is_not_claimant THEN COALESCE(NULLIF(concat(unrecognized_party_details.name, ' ', unrecognized_party_details.last_name), ' '::text), NULLIF(concat(people.first_name, ' ', people.last_name), ' '::text), (bgs_attorneys.name)::text)
                      ELSE concat(veterans.first_name, ' ', veterans.last_name)
                  END, ' '::text), 'claimant'::text) AS claimant_name,
              veterans.participant_id AS veteran_participant_id,
              veterans.ssn AS veteran_ssn
             FROM ((((((((tasks tasks_1
               JOIN supplemental_claims ON (((supplemental_claims.id = tasks_1.appeal_id) AND ((tasks_1.appeal_type)::text = 'SupplementalClaim'::text))))
               JOIN request_issues ON (((request_issues.decision_review_id = supplemental_claims.id) AND ((request_issues.decision_review_type)::text = 'SupplementalClaim'::text))))
               JOIN veterans ON (((veterans.file_number)::text = (supplemental_claims.veteran_file_number)::text)))
               LEFT JOIN claimants ON (((claimants.decision_review_id = tasks_1.appeal_id) AND ((claimants.decision_review_type)::text = (tasks_1.appeal_type)::text))))
               LEFT JOIN people ON (((claimants.participant_id)::text = (people.participant_id)::text)))
               LEFT JOIN unrecognized_appellants ON ((claimants.id = unrecognized_appellants.claimant_id)))
               LEFT JOIN unrecognized_party_details ON ((unrecognized_appellants.unrecognized_party_detail_id = unrecognized_party_details.id)))
               LEFT JOIN bgs_attorneys ON (((claimants.participant_id)::text = (bgs_attorneys.participant_id)::text)))
            GROUP BY tasks_1.id, veterans.participant_id, veterans.ssn, veterans.first_name, veterans.last_name, unrecognized_party_details.name, unrecognized_party_details.last_name, people.first_name, people.last_name, supplemental_claims.veteran_is_not_claimant, bgs_attorneys.name
          UNION ALL
           SELECT tasks_1.id,
              tasks_1.appeal_id,
              tasks_1.appeal_type,
              ...
              COALESCE(NULLIF(
                  CASE
                      WHEN appeals.veteran_is_not_claimant THEN COALESCE(NULLIF(concat(unrecognized_party_details.name, ' ', unrecognized_party_details.last_name), ' '::text), NULLIF(concat(people.first_name, ' ', people.last_name), ' '::text), (bgs_attorneys.name)::text)
                      ELSE concat(veterans.first_name, ' ', veterans.last_name)
                  END, ' '::text), 'claimant'::text) AS claimant_name,
              veterans.participant_id AS veteran_participant_id,
              veterans.ssn AS veteran_ssn
             FROM ((((((((tasks tasks_1
               JOIN appeals ON (((appeals.id = tasks_1.appeal_id) AND ((tasks_1.appeal_type)::text = 'Appeal'::text))))
               JOIN request_issues ON (((request_issues.decision_review_id = appeals.id) AND ((request_issues.decision_review_type)::text = 'Appeal'::text))))
               JOIN veterans ON (((veterans.file_number)::text = (appeals.veteran_file_number)::text)))
               LEFT JOIN claimants ON (((claimants.decision_review_id = tasks_1.appeal_id) AND ((claimants.decision_review_type)::text = (tasks_1.appeal_type)::text))))
               LEFT JOIN people ON (((claimants.participant_id)::text = (people.participant_id)::text)))
               LEFT JOIN unrecognized_appellants ON ((claimants.id = unrecognized_appellants.claimant_id)))
               LEFT JOIN unrecognized_party_details ON ((unrecognized_appellants.unrecognized_party_detail_id = unrecognized_party_details.id)))
               LEFT JOIN bgs_attorneys ON (((claimants.participant_id)::text = (bgs_attorneys.participant_id)::text)))
            GROUP BY tasks_1.id, veterans.participant_id, veterans.ssn, veterans.first_name, veterans.last_name, unrecognized_party_details.name, unrecognized_party_details.last_name, people.first_name, people.last_name, appeals.veteran_is_not_claimant, bgs_attorneys.name) tasks
