class CreateCaseDistributionLevers < Caseflow::Migration
  def change
    create_table :case_distribution_levers, comment:"A generalized table for Caseflow Distribution lever records within Caseflow" do |t|
      ...
      t.string :item, null: false, comment:"A unique value to identify the Caseflow Distribution lever"
      ...
      t.string :value, null: false, comment:"Indicates the value based in the data type whether string/number"
      ...
      t.boolean :is_disabled, null: false, comment:"Used to disable the row"
      ...
      t.json :control_group, null: true, comment:"Indicates the group which contains json formatted data that controls the Case Distribution Levers"
      t.timestamps
    end
  end
end
