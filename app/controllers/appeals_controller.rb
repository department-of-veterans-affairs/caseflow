class AppealsController < ApplicationController
  ...
  def new_documents
    if params[:cached]
      render json: { new_documents: appeal.new_documents_from_caseflow(current_user, params[:altDate]) }
      return
    end
    render json: { new_documents: appeal.new_documents_for_user(current_user, params[:altDate]) }
  rescue StandardError => e
