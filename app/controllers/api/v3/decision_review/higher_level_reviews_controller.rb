...
class Api::V3::DecisionReview::HigherLevelReviewsController < ActionController::Base
  ...
  def create
    ...
    transaction do
      ...
      render error unless intake.review! @preintake.review_params
      render error unless intake.complete! @preintake.complete_params
    end
  ...
  def intake
    @intake ||= Intake.build(
      user: current_user,
      veteran_file_number: @preintake.veteran.file_number,
