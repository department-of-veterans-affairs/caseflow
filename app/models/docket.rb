  ...
  module Scopes
    ...
    def genpop
      joins(with_assigned_distribution_task_sql)
        .where(
          ...
          CaseDistributionLever.find_by_item('cavc_affinity_days').try(:value).days.ago
        )
    end
    ...
    def non_genpop_for_judge(judge)
      joins(with_assigned_distribution_task_sql)
        ...
        .where("distribution_task.assigned_at > ?", CaseDistributionLever.find_by_item('cavc_affinity_days').try(:value).days.ago)
