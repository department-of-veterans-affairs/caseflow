...
class TimedHoldTask < Task
  ...
  validates :days_on_hold, presence: true, inclusion: { in: 1..120 }, on: :create, unless: proc { |task| task.parent.class == PostSendInitialNotificationLetterHoldingTask}
  validates :days_on_hold, presence: true, inclusion: { in: 45..364 }, on: :create, if: proc { |task| task.parent.class == PostSendInitialNotificationLetterHoldingTask}
