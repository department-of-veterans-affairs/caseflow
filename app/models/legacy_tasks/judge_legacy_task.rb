class JudgeLegacyTask < LegacyTask
  def self.from_vacols(record, appeal, user_id)
    ...
    task.assigned_at = record.assigned_to_location_date.to_date
    # If task type is 'assign' that means there was no previous task record yet
    task.previous_task = LegacyTask.new(assigned_at: record.assigned_to_attorney_date.to_date) if task.type == "Review"
