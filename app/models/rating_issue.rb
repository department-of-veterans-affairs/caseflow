class RatingIssue < ApplicationRecord
  ...
  # this local attribute is used to resolve the contesting RequestIssue
  attr_accessor :contention_reference_id

  def save_with_request_issue!
    return unless contesting_request_issue
    self.request_issue = contesting_request_issue
  ...
  def prior_higher_level_review
    return unless reference_id
    return unless contesting_request_issue
    return contesting_request_issue.id if contesting_request_issue.review_request.is_a?(HigherLevelReview)
  end

  def contesting_request_issue
    request_issue || find_contesting_request_issue
  end
  ...
  def find_contesting_request_issue
    return if contention_reference_id.nil?
    @contesting_request_issue ||= RequestIssue.find_by(contention_reference_id: contention_reference_id)
  end
end
