...
class QueueColumn
  ...
  def to_hash(tasks, assignee_is_business_line)
    {
      name: name,
      filterable: filterable,
      filter_options: filterable ? filter_options(tasks, assignee_is_business_line) : []
    }
  end

  def filter_options(tasks, assignee_is_business_line)
    case name
    ...
    when Constants.QUEUE_CONFIG.COLUMNS.TASK_TYPE.name
      task_type_options(tasks, assignee_is_business_line)
  ...
  def task_type_options(tasks, assignee_is_business_line)
    group_by_column = assignee_is_business_line ? :task_type : :type

    tasks.group(group_by_column).count.each_pair.map do |option, count|
