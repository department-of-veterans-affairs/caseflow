...
class Document < CaseflowRecord
  ...
  def prepare_metadata_from_document(source_document)
    self.documents_tags.build(source_document.documents_tags.map {|tag| tag.dup.attributes} )
    self.annotations.build(source_document.annotations.map {|annotation| annotation.dup.attributes})
    assign_attributes(
