...
class Api::Preintake::RequestIssue
  ...
  attr_reader :notes, :decision_id, :request_issue_ids, :rating_id, :legacy_id, :benefit_type, :category, :decision_date, :decision_text

  def initialize(options)
    contests = options["contests"]
    options = options.except "contests"
    case contests
    when "on_file_decision"; validate_for_contesting_decision options
    when "on_file_rating"; validate_for_contesting_rating options
    when "on_file_legacy_issue"; validate_for_contesting_legacy_issue options
    when "other"; validate_for_contesting_other options
    else
      raise ArgumentError, "cannot contest: <#{contests}>" if exception
    end 
  end
