...
class Distribution < CaseflowRecord
  ...
  def distribute!(limit = nil)
    ...
    transaction_time_out = CaseDistributionLever.find_by_item('distribute_time_out').try(:value) #3 * 60 * 1000
  ...
  def judge_has_eight_or_fewer_unassigned_cases
    return false if judge_tasks.length > CaseDistributionLever.find_by_item('request_more_cases_minimum').try(:value)

    judge_tasks.length + judge_legacy_tasks.length <= CaseDistributionLever.find_by_item('request_more_cases_minimum').try(:value)
  end
  ...
  def batch_size
    ...
    return CaseDistributionLever.find_by_item('alternative_batch_size').try(:value) if team_batch_size.nil? || team_batch_size == 0

    team_batch_size * CaseDistributionLever.find_by_item('batch_size_per_attorney').try(:value)
  end
