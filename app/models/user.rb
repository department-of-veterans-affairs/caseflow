...
class User < CaseflowRecord # rubocop:disable Metrics/ClassLength
  ...
  def inbound_ops_team_superuser?
    member_of_organization?(InboundOpsTeam.singleton) &&
      OrganizationUserPermissionChecker.new.can?(permission_name: Constants.ORGANIZATION_PERMISSIONS.superuser, organization: InboundOpsTeam.singleton, user: self)
  end
