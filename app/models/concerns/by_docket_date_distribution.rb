...
module ByDocketDateDistribution
  ...
  def priority_push_distribution(limit)
    ...
    @rem = CaseDistributionLever.find_by_item('priority_push_distribution').try(:value)
    @appeals = []
  ...
  def requested_distribution
    @appeals = []
    @rem = batch_size
    @nonpriority_iterations = CaseDistributionLever.find_by_item('priority_push_distribution').try(:value)
    ...
    unless FeatureToggle.enabled?(:acd_disable_nonpriority_distributions, user: RequestStore.store[:current_user])
      ...
      distribute_nonpriority_appeals_from_all_dockets_by_age_to_limit(@rem) until @rem <= CaseDistributionLever.find_by_item('priority_push_distribution').try(:value)
    end
    @appeals
  end
