...
class VhaProgramOfficeCompletedTasksTab < QueueTab
    ...
    delegate :column_names, to: :vha_program_offices
    ...
    def tasks
    Task.includes(*task_includes).visible_in_queue_table_view.closed.where(assigned_to: VhaProgramOffice.all.map(&:id))
    end
