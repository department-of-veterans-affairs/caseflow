...
class RequestIssueContention
  ...
  def update_text!
    ...
    unless vbms_contention
      Raven.capture_exception("ContentionNotFound: Contention reference id #{contention_reference_id} not found while updating request issue.")
      self.error_id = Raven.last_event_id || "000000000000000001234567890abcdefg"
      Rails.logger.error("ContentionNotFound(#{contention_reference_id}): Intake error (Sentry event #{error_id}): #{error}\n" + error.backtrace.join("\n"))
