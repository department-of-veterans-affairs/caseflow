  ...
  class << self
    ...
    def update_schedule(updated_hearings)
      updated_hearings.each do |hearing_hash|
        ...
        if hearing_to_update.is_a?(HearingDay)
          update_hash = {judge: User.find_by_css_id_or_create_with_default_station_id(hearing_hash[:css_id])}
        else
          update_hash = {judge_id: hearing_hash[:judge_id]}
        end
