...
class DocumentSeriesIdAssigner
  ...
  def call
    ...
    Rails.logger.info("Docs with no series id: #{documents_with_no_series_id}.")
  ...
  def documents_to_update
    documents_with_no_series_id.map do |document|
      Rails.logger.info("Document before documents_to_update: #{document}. vbms_doc_id: #{document.vbms_document_id}")
      ...
      Rails.logger.info("Document after documents_to_update: #{document}. document.series_id: #{document.series_id}")
      document
    end
  end
  ...
  def document_id_to_series_hash
    @document_id_to_series_hash ||= begin
      ...
      document_series.each do |doc|
        Rails.logger.info("doc.series_id: #{doc.series_id}")
