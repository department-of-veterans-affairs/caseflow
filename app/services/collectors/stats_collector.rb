...
module Collectors::StatsCollector
      ...
      stats_hash.each do |metric_name, counts_hash|
        unless valid_metric_name?(metric_name)
          fail "Invalid metric name #{metric_name}; "\
            "see https://docs.datadoghq.com/developers/metrics/#naming-custom-metrics" # review with Noelle
        end
  ...
  # See valid tag name rules at https://docs.datadoghq.com/tagging/#defining-tags # review with Noelle
  ...
  def valid_metric_name?(metric_name)
    # Actual limit is 200 but since the actual metric name in DataDog has # review with Noelle
