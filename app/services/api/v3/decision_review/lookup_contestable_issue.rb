...
class Api::V3::DecisionReview::LookupContestableIssue
  class NoIds < StandardError
    def error_code
      :contestable_issue_must_have_at_least_one_of_rating_decision_or_ratingdecision_id
    end
  end
  ...
  def initialize(
    ...
    fail NoIds unless @rating_issue_id.present? || @decision_issue_id.present? || @rating_decision_issue_id.present?
  end
  ...
  def contestable_issue
    @contestable_issue ||= contestable_issues_for_veteran_and_form_type.find do |ci|
      matches_provided_ids? ci
    end
  end

  private

  def matches_provided_ids?(contestable_issue)
