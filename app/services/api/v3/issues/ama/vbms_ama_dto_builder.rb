...
class Api::V3::Issues::Ama::VbmsAmaDtoBuilder
  ...
  def initialize(veteran, page, per)
    @page = page
    @per = per
    ...
    @total_number_of_pages = (@request_issue_count / @per.to_f).ceil
  ...
  def serialized_request_issues(page = @page, per = @per)
  ...
  def build_hash_response
    if @page > @total_number_of_pages
      ...
      @request_issues = serialized_request_issues(@total_number_of_pages, @per)
    end
    json_response
  end
  ...
  def json_response
    {
      ...
      "max_request_issues_per_page": @per,
