...
class AppealRepository
  ...
  def self.issues(vacols_id)
    issues_hash_array = VACOLS::CaseIssue.descriptions([vacols_id])[vacols_id]
    issues_hash_array ? issues_hash_array.map { |issue_hash| Issue.load_from_vacols(issue_hash) } : []
  end
