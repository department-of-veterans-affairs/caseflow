...
class AppealRepository
  ...
  def self.get_vacols_reptype_code(short_name:)
    VACOLS::Case::REPRESENTATIVES.each do |representative|
      if representative[1][:short] == short_name
  ...
  def self.is_it_a_first_last_name?(representative_name:)
    return representative_name.split(' ').length == 2 &&
        !representative_name.include?('-') &&
        !representative_name.include?('&')
  end

  def self.is_it_a_first_middle_last_name?(representative_name:)
    return representative_name.split(' ').length == 3 && representative_name.split(' ')[1].tr('.', '').length == 1
  end
  ...
  def self.update_vacols_rep_info!(appeal:, representative_type:, representative_name:)
    ...
    if Appeal.repository.is_it_a_first_last_name?(representative_name: representative_name)
    ...
    elsif Appeal.repository.is_it_a_first_middle_last_name?(representative_name: representative_name)
      VACOLS::Representative.update_vacols_rep_name!(
        case_record: appeal.case_record,
        first_name: representative_name.split(' ')[0],
