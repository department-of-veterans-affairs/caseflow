...
class SearchQueryService::AppealRow
  ...
  def attributes
    SearchQueryService::Attributes.new(
      ...
      withdrawn: withdrawn,
      unrecognized_appellant_id: unrecognized_appellant_id
    )
  end
  # rubocop:enable Metrics/AbcSize
  # rubocop:enable Metrics/MethodLength

  def unrecognized_appellant_id
    appellant&.unrecognized_claimant? ? appellant&.unrecognized_appellant&.id : nil
  end
  ...
  def appellant_party_type
    appellant&.unrecognized_claimant? ? appellant.party_type : nil
  end

  def appellant_phone_number
    appellant&.unrecognized_claimant? ? appellant.phone_number : nil
  end

  def aod
    appellant&.advanced_on_docket_based_on_age? || query_row["aod_granted_for_person"]
  end
  ...
  def appellant_full_name
    FullName.new(
      query_row["person_first_name"].to_s.capitalize,
      query_row["person_middle_name"].to_s.first.upcase,
      query_row["person_last_name"].to_s.capitalize
    ).formatted(:readable_full_nonformatted)
  end

  def veteran_full_name
    FullName.new(
      query_row["veteran_first_name"].to_s.capitalize,
      query_row["veteran_middle_name"].to_s.first.upcase,
      query_row["veteran_last_name"].to_s.capitalize
    ).formatted(:readable_full_nonformatted)
  end
