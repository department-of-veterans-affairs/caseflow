...
class ExternalApi::EfolderService
  ...
  def self.fetch_documents_for(appeal, user)
    ...
    if FeatureToggle.enabled?(:efolder_api_v2, user: user)
      efolder_v2_api(sanitized_vbms_id, user)
    else
      efolder_v1_api(sanitized_vbms_id, user)
    end
  end

  def self.efolder_v1_api(sanitized_vbms_id, user)
    headers = { "FILE-NUMBER" => sanitized_vbms_id }

    response = get_efolder_response("/api/v1/files?download=true", user, headers)

    if response.error?
      err_msg = "eFolder HTTP status code: #{response.code} for VBMS ID: #{sanitized_vbms_id}."
  ...
  def self.efolder_v2_api(sanitized_vbms_id, user)
    headers = { "FILE-NUMBER" => sanitized_vbms_id }
