...
class ExternalApi::VBMSService
  ...
  def self.associate_rating_request_issues!(claim_id:, rating_request_issue_contention_map:)
    # rating_request_issue_contention_map format: { issue_id: contention_id, issue_id2: contention_id2 }
    @vbms_client ||= init_vbms_client

    request = VBMS::Requests::AssociateRatedIssues.new(
      claim_id: claim_id,
      rated_issue_contention_map: rating_request_issue_contention_map
    )
