...
class Events::DecisionReviewCreated::DecisionReviewCreatedParser
  ...
  # Validation methods to check for required fields
  ...
  def intake_valid?
    intake.present? &&
    intake_started_at.present? &&
    intake_completion_started_at.present? &&
    intake_completion_status.present? &&
    intake_type.present? &&
    intake_detail_type.present?
  end

  def veteran_valid?
    veteran.present? &&
    veteran_participant_id.present? &&
    veteran_bgs_last_synced_at.present? &&
    veteran_file_number.present? &&
    veteran_ssn.present? &&
    veteran_first_name.present? &&
    veteran_last_name.present?
  end

  def claimant_valid?
    claimant.present? &&
    claimant_payee_code.present? &&
    claimant_type.present? &&
    claimant_participant_id.present? &&
    claim_review_valid?
  end

  def claim_review_valid?
    claim_review.present? &&
    claim_review_benefit_type.present? &&
    veteran_file_number.present?
  end

  def epe_valid?
    epe.present? &&
    epe_payee_code.present? &&
    claim_review_valid?
  end
