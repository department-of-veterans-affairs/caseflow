...
class HearingDaySerializer
  ...
  attribute :judge_first_name do |hearing_day, params|
    RequestStore[:current_user]&.board_hearings_employee? ? get_judge_first_name(hearing_day, params) : nil
  end
  attribute :judge_id
  attribute :judge_css_id
  attribute :judge_last_name do |hearing_day, params|
    RequestStore[:current_user]&.board_hearings_employee? ? get_judge_last_name(hearing_day, params) : nil
  end
  attribute :lock
  attribute :notes do |hearing_day|
    RequestStore[:current_user]&.board_hearings_employee? ? hearing_day.notes : nil
  end
