...
class ReturnedAppealsToBoardQuery
  ...
  def self.generate_rows(record, is_return_appeal = false)
    if is_return_appeal
      HEADERS.keys.map { |key| record[key] }
    else
      HEADERS.reject { |key, _| key == :bfcurloc }.keys.map { |key| record[key] }
    end
  end

  def self.process(is_return_appeal: false)
    ...
    CSV.generate(headers: true) do |csv|
      ...
      if is_return_appeal
        ...
        csv << HEADERS.reject { |key, _| key == :bfcurloc }.values
      end

      # Iterate through results and add each row to CSV
      ready_appeals.each do |record|
        csv << generate_rows(record, is_return_appeal)
      end
    end
  end
  ...
  def self.ready_appeals
    ...
    docket_coordinator.dockets
      .flat_map do |sym, docket|
        ...
        appeals = appeals.select { |appeal| returned_appeal_job.returned_appeals.include?(appeal["bfkey"]) }
        if sym == :legacy
  ...
  def self.legacy_rows(appeals, sym)
    appeals.map do |appeal|
...

      {
        ...
        bfcurloc: "Test" #find this param to get the value
      }
    end
  end
