...
class ETL::UnknownStatusWithCompletedRootTaskQuery
  ...
  def appeals
    ETL::Appeal.where(status: "UNKNOWN").where(appeal_id: completed_root_tasks).where(appeal_id: open_child_tasks)
  end

  def completed_root_tasks
    ETL::Task.select(:appeal_id)
      .where(appeal_type: "Appeal", task_type: "RootTask", task_status: ["completed", "cancelled"])
  end
