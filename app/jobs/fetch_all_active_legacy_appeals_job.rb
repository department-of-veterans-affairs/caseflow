...
class FetchAllActiveLegacyAppealsJob < CaseflowJob
    ...
    Task.where(
      ...
      root_tasks.each do |root_task|
        if root_task.appeal.nil?
          Rails.logger.error("FetchAllActiveLegacyAppealsJob::Error - Root Task ID #{root_task&.id} has no "\
  ...
  def add_record_to_appeal_states_table(appeal)
    begin
      ...
      Rails.logger.error("FetchAllActiveLegacyAppealsJob::Error - An Appeal State record for #{appeal&.class} ID "\
