...
class DownloadTranscriptionFileJob < CaseflowJob
  ...
  retry_on(FileDownloadError) do |job, exception|
    extra = {
  ...
  def download_file_to_tmp(link)
    ...
    begin
      ...
      raise FileDownloadError, error.message
    end
  end
  ...
  def upload_file_to_s3
    begin
      ...
      raise FileUploadError, error.message
    end
  end
  ...
  def s3_success_message
    "TranscriptionFile (id: #{@transcription_file.id}) successfully uploaded to S3 location: '#{s3_file_location}'"
  end
  ...
  def file_type_invalid?
    ...
    fail FileNameError, "Invalid file type"
  end
  ...
  def parse_hearing
    begin
      ...
      raise FileNameError, "File name missing sufficient hearing identifiers"
    end
  end
