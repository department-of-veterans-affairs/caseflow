...
class VaBoxUploadJob < CaseflowJob
  ...
  def perform(file_info, box_folder_id)
    ...
    file_info[:hearings].each do |hearing|
      begin
        ...
        upload_to_box(box_service, local_file_path, child_folder_id, transcription_package, file_info, hearing)
  ...
  def upload_to_box(box_service, local_file_path, child_folder_id, transcription_package, file_info, hearing)
    byebug
    ...
    # TODO transcription_files
    # TODO transcription_files

      transcription = Transcription.find_by(task_number: file_info[:work_order_name])
