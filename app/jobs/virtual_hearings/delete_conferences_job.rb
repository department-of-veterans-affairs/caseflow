  ...
  def count_deleted_and_log(enumerable)
    ...
    if removed > 0
      MetricsService.increment_counter(
        metric_name: "deleted_conferences.successful", by: removed, **metrics_service_metric_info
      )
    end

    if failed > 0
      MetricsService.increment_counter(
        metric_name: "deleted_conferences.failed", by: failed, **metrics_service_metric_info
      )
    end
  end
