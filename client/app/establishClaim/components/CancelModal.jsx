...
const CANCEL_ERRORS = {
  ...
  default: {
    header: 'Error',
    body: 'There was an error while cancelling the current claim. Please try again later'
  }
};
      ...
      {isShowingCancelModal && (
        ...
        >
          <p>
            ...
            button below your work will not be saved and an EP will not be created for this claim.
          </p>
          <p>Please tell us why you have chosen to discontinue processing this claim.</p>
          <TextareaField
            label="Explanation"
            name="Explanation"
...
const mapDispatchToProps = (dispatch, ownProps) => ({
  ...
  handleCancelSubmit: () =>
    dispatch((_dispatch, getState) => {
      ...
      const isInvalid = Boolean(getCancelFeedbackErrorMessage(getState().establishClaim));
      ...
      ApiUtil.patch(`/dispatch/establish-claim/${ownProps.taskId}/cancel`, { data }).then(
        () => {
        ...
        (error) => {
          ...
          const errorMessage = CANCEL_ERRORS[error?.response?.body?.error_code || 'default'];

          ownProps.handleAlert('error', errorMessage.header, errorMessage.body);
        }
      );
    }),
