...
export default class OrganizationUsers extends React.PureComponent {
  ...
  dropdownOptions = () => {
    return this.state.remainingUsers.map((user) => {
      return { label: this.formatName(user), value: user };
    });
  };
  ...
  removeUser = (user) => () => {
    this.setState({
      removingUser: { ...this.state.removingUser, [user.id]: true }
    });

    ApiUtil.delete(`/organizations/${this.props.organization}/users/${user.id}`).then((response) => {
      this.setState({
        ...
        removingUser: { ...this.state.removingUser, [user.id]: false }
      });
    }, (error) => {
      this.setState({
        removingUser: { ...this.state.removingUser, [user.id]: false },
