...
export default class ReviewAssignments extends React.Component {
  getAlertTitle = () => {
    if (
      this.props.schedulePeriod.type ===
      SPREADSHEET_TYPES.RoSchedulePeriod.value
    ) {
      return COPY.HEARING_SCHEDULE_REVIEW_ASSIGNMENTS_ALERT_TITLE_ROCO;
    }
    if (
      this.props.schedulePeriod.type ===
      SPREADSHEET_TYPES.JudgeSchedulePeriod.value
    ) {
  ...
  modalMessage = () => {
    return (
      <div>
        ...
        <p>
          <b>Date range: </b>
          {this.props.schedulePeriod.startDate} to{' '}
          {this.props.schedulePeriod.endDate}
        </p>
      </div>
    );
  };
  ...
  render() {
    ...
    if (this.props.schedulePeriodError) {
      ...
      if (spErrorDetails) {
        if (
          this.props.spErrorDetails.type ===
          SPREADSHEET_TYPES.RoSchedulePeriod.value
        ) {
          message = (
            <span>
              You have allocated too many hearing days to the{' '}
              {spErrorDetails.details.ro_key}, the maximum number of allocations
              is {spErrorDetails.details.max_allocation}.<br />
              Please check your spreadsheet and upload the file again using the
              "Go back" link below.
              <br />
          ...
          message = (
            <span>
              We could not assign a judge to every hearing day. Please check the
              following dates in
              <br />
              your file and try again using the "Go back" link below:
              <br />
    ...
    const hearingAssignmentRows = _.map(
      this.props.schedulePeriod.hearingDays,
      (hearingDay) => ({
