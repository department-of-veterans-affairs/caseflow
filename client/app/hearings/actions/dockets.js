...
export const updatedState = (action, milliseconds = 1000) =>
  (dispatch, getState) => {
    ...
    updateUtil.lastChangeTimestamp = Number(new Date());
    ...
    eval(`
      setTimeout(() => {
        if (${updateUtil.lastChangeTimestamp} === updateUtil.lastChangeTimestamp) {
          dispatch({type:'TOGGLE_SAVING'});

          saveData(
            updateUtil.diffObjects(updateUtil.lastSave, getState().save),
            dispatch,
            action.payload.apiUrl
          );
          
          updateUtil.lastSave = getState().save;
        }
      }, ${milliseconds});`
    );
