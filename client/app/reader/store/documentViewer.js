 ...
 */
export const searchText = createAsyncThunk('documentViewer/search', async ({ matchIndex, searchTerm, docId }) => {
...
export const showPdf = createAsyncThunk(
  'documentViewer/show',
  async ({ rotation = null, pageNumber, currentDocument, scale }, { dispatch }) => {
    ...
    // Store the Viewport
    pdfDocuments[currentDocument.id].viewport = pdfDocuments[currentDocument.id].pages.reduce(
      (viewport, page) => ({
...
export const handleCategoryToggle = createAsyncThunk(
  ...
  async ({ docId, categoryKey, toggleState }) => {
    ...
    // Request the addition of the selected tags
    await ApiUtil.patch(`/document/${docId}`, { data: { [category]: toggleState } }, ENDPOINT_NAMES.DOCUMENT);
  ...
  reducers: {
    ...
    toggleShareModal: {
      ...
      prepare: (shareCommentId) =>
        addMetaLabel(shareCommentId ? 'open-annotation-share-modal' : 'close-annotation-share-modal', shareCommentId),
    },
    toggleDeleteModal: {
      reducer: (state, action) => {
      ...
      prepare: (deleteCommentId) =>
        addMetaLabel(
          deleteCommentId ? 'open-annotation-delete-modal' : 'close-annotation-delete-modal',
          deleteCommentId
        ),
    },
    toggleAccordion: {
