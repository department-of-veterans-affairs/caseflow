...
import { filter, get, map, noop, sum } from 'lodash';
import * as PDFJS from 'pdfjs-dist';
import { connect } from 'react-redux';
import { bindActionCreators } from 'redux';
import CommentLayer from '../reader/CommentLayer';
import { PAGE_DIMENSION_SCALE, PAGE_MARGIN, PDF_PAGE_HEIGHT, PDF_PAGE_WIDTH, SEARCH_BAR_HEIGHT } from '../reader/constants';
import { setSearchIndexToHighlight } from '../reader/PdfSearch/PdfSearchActions';
import { setDocScrollPosition } from '../reader/PdfViewer/PdfViewerActions';
import { getCurrentMatchIndex, getMatchesPerPageInFile, getSearchTerm } from '../reader/selectors';
...
export class R2PdfPage extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  setUpPage = () => {
    ...
    if (this.props.pdfDocument && !this.props.pdfDocument._transport.destroyed) {
      try {
        this.page = this.props.pageResult;
        const textResult = this.getText(this.props.pageResult);

        textResult.then((text) => {
          this.drawText(this.props.pageResult, text);
        });
        this.drawPage(this.props.pageResult).then();

      } catch (err) {
        console.error(`readerlog error with pdfjs page: ${err}`);
      }
    }
  };
  ...
  render() {
    ...
    return (
      <div style={divPageContainerStyle}>
        <div
