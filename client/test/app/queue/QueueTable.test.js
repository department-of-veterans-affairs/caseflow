...
describe("QueueTable", () => {
  describe("Local Methods", () => {
    ...
    test("function cellClasses({ align, cellClass}) returns classes based on alignment", () => {
      ...
      expect(classnames).toHaveBeenCalledWith([
        queueTable.helperClasses.right,
        undefined,
      ]);
      ...
      expect(classnames).toHaveBeenCalledWith([
        queueTable.helperClasses.center,
        undefined,
      ]);
  ...
  describe("HeaderRow", () => {
    ...
    test("Matches snapshot with default props", () => {
      ...
      const { asFragment } = render(
        <HeaderRow columns={columns} rowObjects={createTask(3)} />
      );
    ...
    test("Can sort when not using the tasks API and getting column sort value", () => {
      ...
      // Test sorting colors
      expect(screen.getByTestId("topColorGroup")).toHaveAttribute(
        "fill",
        COLORS.GREY_LIGHT
      );
      expect(screen.getByTestId("bottomColorGroup")).toHaveAttribute(
        "fill",
        COLORS.GREY_LIGHT
      );
    ...
    test("Renders tooltip when present on the column", () => {
      ...
      expect(
        screen.getByText(columnsWithTooltip[2].header)
      ).toBeInTheDocument();
    });
  });
  ...
  describe("BodyRows", () => {
    ...
    test("Matches snapshot with default props", () => {
      ...
      times(tableData.length).map((index) => {
        expect(getKeyForRow).toHaveBeenNthCalledWith(
          index + 1,
          index,
          tableData[index]
        );
  ...
  describe("FooterRow", () => {
    ...
    test("Matches snapshot with default props", () => {
      ...
      const { container, asFragment } = render(
        <FooterRow columns={footerColumns} />
      );
    ...
    test("Renders no footer when not present in the column", () => {
      ...
      expect(container.querySelector("tfoot")).toBeInTheDocument();
      expect(
        container.querySelector("#table-row-footer")
  ...
  describe("Class Methods", () => {
    ...
    test("function initialState(paginationOptions) returns state with loading component when loading", () => {
      ...
      const taskPagesApiEndpoint =
        "https://example.com/api/taskPages?param=value";
      // Setup the test
      render(
        <QueueTable
          columns={columns}
      ...
      const {
        props: { spinnerColor },
      } = memoizedState.loadingComponent;
  ...
  describe("render()", () => {
    test("Matches snapshot with default props", () => {
      ...
      // // Negative tests
      expect(
        screen.queryByTestId("table-filter-testid")
      ).not.toBeInTheDocument();
    ...
    test("Renders loading component instead of body when loading data", () => {
      ...
      expect(
        container.querySelector(".cf-loading-button-symbol")
      ).toBeInTheDocument();
      expect(
        container.querySelector(".cf-react-icon-loading-front")
      ).toBeInTheDocument();
      expect(
        container.querySelector(".cf-react-icon-loading-back")
      ).toBeInTheDocument();
    });
    ...
    test("Can filter rows", () => {
      ...
      expect(
        container.querySelector(".cf-dropdown-filter")
      ).toBeInTheDocument();
  ...
  describe("DatePicker filter", () => {
    ...
    describe("Front end filtering", () => {
      ...
      it("filters between dates", async () => {
        ...
        expect(
          screen.getAllByText("Viewing 1-15 of 20 total")[0]
        ).toBeInTheDocument();
        ...
        expect(
          screen.getAllByText("Viewing 1-10 of 10 total")[0]
        ).toBeInTheDocument();
      ...
      it("filters before date", async () => {
        ...
        expect(
          screen.getAllByText("Viewing 1-9 of 9 total")[0]
        ).toBeInTheDocument();
      ...
      it("filters after date", async () => {
        ...
        expect(
          screen.getAllByText("Viewing 1-5 of 5 total")[0]
        ).toBeInTheDocument();
      ...
      it("filters on date", async () => {
        ...
        expect(
          screen.getAllByText("Viewing 1-1 of 1 total")[0]
        ).toBeInTheDocument();

        expect(getTableDates(container)).toEqual(["5/15/2024"]);
      });
    ...
    describe("Back end filtering", () => {
      beforeEach(async () => {
        ...
        when(ApiUtil.get)
          .calledWith("/example/endpoint?&page=1")
          .mockResolvedValue({
            body: {
              task_page_count: 1,
              tasks: {
        ...
        when(ApiUtil.get)
          ...
          .mockResolvedValue({
            body: {
              task_page_count: 1,
              tasks: {
                data: [records[0], records[1], records[2]],
              },
              tasks_per_page: 15,
              total_task_count: 3,
            },
      ...
      it("updates url params correctly on apply and reset", async () => {
        ...
        await waitFor(() =>
          expect(
            screen.getAllByText("Viewing 1-3 of 3 total")[0]
          ).toBeInTheDocument()
        );
        ...
        expect(ApiUtil.get).toHaveBeenLastCalledWith(
