...
describe('ReturnToQueueModal rendering', () => {
  it('renders the return to queue modal', () => {
    ...
    expect(screen.getByText('Select whether to cancel the intake of this mail package' +
        ' or resume the intake process at a later date.')).toBeInTheDocument();
  ...
  it('Radio selection', () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent('Saving the intake form to continue it at a' +
        ' later date will resume the intake form at step three of the process');
  });
