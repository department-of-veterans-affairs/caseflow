...
describe('BuildScheduleUpload', () => {
  it('does not show the date selector when file type is null', () => {
    ...
    expect(wrapper.text()).toEqual(expect.arrayContaining(['Upload Files']));
    expect(wrapper.text()).toEqual(
      expect.arrayContaining(['Please select the file you are uploading and choose a date range.'])
    );
    expect(wrapper.text()).toEqual(expect.not.arrayContaining(['Please input a date range']));
  });
  ...
  it('shows the date selector when file type is \'Judge\'', () => {
    ...
    expect(wrapper.text()).toEqual(expect.arrayContaining(['What are you uploading?']));
  });
  ...
  it('displays errors when set', () => {
    ...
    expect(wrapper.text()).toEqual(expect.arrayContaining(['The template was not followed.']));
  });
});
