...
describe('AnnotationLayerReducer reducer', () => {
  ...
  describe(Constants.REQUEST_CREATE_ANNOTATION_FAILURE, () => {
    ...
    it('annotation stay pending when the annotation creation fails', () => {
      ...
      expect(state.placedButUnsavedAnnotation).toEqual(annotation);
    });
    ...
    it('annotation becomes pending when annotation creation requested', () => {
      ...
      expect(nextState.placedButUnsavedAnnotation).toEqual(null);
      expect(nextState.pendingAnnotations[annotationId]).toEqual(annotation);
    });
  });
  ...
  describe(Constants.REQUEST_DELETE_ANNOTATION_FAILURE, () => {
    ...
    it('marks an annotation as pending deletion', () => {
      ...
      expect(stateAfterDeleteRequest.annotations[annotationId].pendingDeletion).toBe(true);
    });
  });
  ...
  describe(Constants.REQUEST_EDIT_ANNOTATION_FAILURE, () => {
    ...
    it('handles editing an annotation', () => {
      ...
      expect(stateAfterEditRequest.pendingEditingAnnotations[annotationId]).toEqual({
        id: annotationId,
        uuid: annotationId,
        comment: newContent,
        documentId
    ...
    it('pending edit stays when the request fails', () => {
      ...
      expect(stateAfterEditFailure.editingAnnotations[annotationId]).toEqual({
        id: annotationId,
        uuid: annotationId,
        comment: newContent,
        documentId
  ...
  describe(Constants.REQUEST_MOVE_ANNOTATION_FAILURE, () => {
    ...
    it('handles moving an annotation', () => {
      ...
      expect(stateAfterMoveRequest.pendingEditingAnnotations[annotation.id]).toEqual(movedAnnotation);
    });

    it('no edits pending with the request fails', () => {
      const { stateAfterMoveFailure, annotation } = getContext();

      expect(stateAfterMoveFailure.pendingEditingAnnotations[annotation.id]).toBeUndefined();
    });

    it('updates the annotation when the request succeeds', () => {
      const { stateAfterMoveSuccess, annotation, movedAnnotation } = getContext();

      expect(stateAfterMoveSuccess.pendingEditingAnnotations[annotation.id]).toBeUndefined();
      expect(stateAfterMoveSuccess.annotations[annotation.id]).toEqual(movedAnnotation);
    });
  });
  ...
  describe(Constants.REQUEST_CREATE_ANNOTATION_SUCCESS, () => {
    it('updates annotations when the server save is successful', () => {
      ...
      expect(stateWithNextAnnotation.annotations).toEqual({
        [annotationId]: {
          id: annotationId,
          uuid: annotationId,
          documentId: docId,
