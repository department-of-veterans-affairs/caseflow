...
describe('schema', () => {
  describe('useAmaActivationDate', () => {
    it('is valid after feb 19, 2019', async () => {
      ...
      await assertValidSchema(TestableSchema, validSchema, REVIEW_OPTIONS.APPEAL.key, true, true)
    })
    it('is invalid before feb 19, 2019', async () => {
      const invalidSchema = { receiptDate: BEFORE_AMA_DATE }
      await assertValidSchema(TestableSchema, invalidSchema, REVIEW_OPTIONS.APPEAL.key, true, false)
    })
    it('is valid before feb 19, 2019 if not appeal', async () => {
      const validSchema = { receiptDate: BEFORE_AMA_DATE }
      await assertValidSchema(TestableSchema, validSchema, 'not_appeal', true, true)
    })
  })
  describe('!useAmaActivationDate', () => {
    it('is valid after nov 1, 2017', async () => {
      const validSchema = { receiptDate: AFTER_AMA_TEST_DATE }
      await assertValidSchema(TestableSchema, validSchema, REVIEW_OPTIONS.APPEAL.key, false, true)
    })
    it('is invalid before nov 1, 2017', async () => {
      const invalidSchema = { receiptDate: BEFORE_AMA_TEST_DATE }
      await assertValidSchema(TestableSchema, invalidSchema, REVIEW_OPTIONS.APPEAL.key, false, false)
    })
